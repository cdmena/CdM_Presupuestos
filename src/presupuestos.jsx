...
const Icon = ({ as: Component, size = 14, color = "currentColor" }) =>
  Component ? (
    <Component
      size={size}
      color={color}
...
function matchTarifaHeaders(headers) {
  ...
  headers.forEach((header, idx) => {
    ...
    const exacta = TARIFA_COLS_VALIDAS.find(
      (col) => normalizar(col.key) === norm
    );
    if (exacta) {
      mapping[idx] = exacta.key;
      return;
    }
    // Si no, buscar campos cuyo nombre real EMPIECE por el texto de la cabecera
    const candidatos = TARIFA_COLS_VALIDAS.filter((col) =>
      normalizar(col.key).startsWith(norm)
    );
    if (candidatos.length === 1) mapping[idx] = candidatos[0].key;
    else if (candidatos.length > 1)
      errores.push(
        `Columna "${header}" es ambigua, podría ser: ${candidatos
          .map((c) => c.key)
          .join(", ")}`
      );
...
function TarifasSection({ setStatus }) {
  ...
  const procesar = (aoa, origen) => {
    if (aoa.length === 0) {
      setErroresMapeo(["El fichero está vacío"]);
      return;
    }
    ...
    const erroresExtra = [];
    if (!tieneRef)
      erroresExtra.push("Falta columna 'Referencia' (obligatoria)");
    if (numColumnas < 2)
      erroresExtra.push(
        "El fichero debe tener al menos 2 columnas (referencia + otra)"
      );
  ...
  const addLog = (texto, tipo = "info") => {
    setLog((l) => [
      ...l,
      { texto, tipo, hora: new Date().toLocaleTimeString("es-ES") },
    ]);
  };
  ...
  const aplicarTarifa = async () => {
    ...
    Object.entries(mapping).forEach(([idx, key]) => {
      idxPorClave[key] = Number(idx);
    });
    const columnasDetectadas = Object.keys(idxPorClave);

    if (!columnasDetectadas.includes("referencia")) {
      addLog(
        "ERROR: el fichero debe tener una columna 'Referencia'. Se cancela el proceso.",
        "error"
      );
    ...
    if (columnasDetectadas.length < 2) {
      addLog(
        "ERROR: el fichero debe tener al menos 2 columnas (referencia + otra). Se cancela el proceso.",
        "error"
      );
      setStatus &&
        setStatus("El fichero debe tener al menos 2 columnas", "error");
      return;
    }
    ...
    addLog(
      `Procesando fichero "${fileData.origen || "(desconocido)"}"`,
      "info"
    );
    addLog(
      `Iniciando: ${
        fileData.rows.length
      } filas. Columnas detectadas: ${columnasDetectadas.join(", ")}`,
      "info"
    );
    ...
    if (idxPorClave.grupodescuento !== undefined) {
      try {
        ...
        if (r.ok) {
          ...
          addLog(
            `Cargados ${gruposDesc.length} grupos descuento de BD`,
            "info"
          );
        }
      } catch (e) {
        addLog(
          "AVISO: no se pudieron cargar grupos descuento de BD: " + e.message,
          "warning"
        );
      }
    }
    ...
    const resolverIdGrupo = (codigo) => {
      ...
      const g = gruposDesc.find(
        (x) =>
          x.grupodescuentospain &&
          String(x.grupodescuentospain).toUpperCase().trim() === c
      );
    ...
    for (let i = 0; i < fileData.rows.length; i++) {
      if (stopRequested.current) {
        addLog(
          `⏹ Detenido por el usuario en la fila ${i + 1} de ${
            fileData.rows.length
          }`,
          "warning"
        );
        ...
        if (codigo) {
          ...
          if (idg === undefined) {
            addLog(
              `Fila ${
                i + 1
              }: ${referencia} → ERROR grupo descuento "${codigo}" no existe en BD`,
              "error"
            );
            errorGrupo = true;
          } else {
      ...
      try {
        const resBuscar = await fetch(
          `${API_URL}/productos/referencia/${encodeURIComponent(referencia)}`
        );
        ...
        if (resBuscar.ok) {
          ...
          if (!sobreescribir) {
            addLog(
              `Fila ${
                i + 1
              }: ${referencia} → ya existe, se omite (sobreescribir = NO)`,
              "info"
            );
            continue;
          }
          ...
          if (Object.keys(datosExcel).length === 0) {
            addLog(
              `Fila ${
                i + 1
              }: ${referencia} → solo se aportó referencia, no hay nada que actualizar`,
              "info"
            );
            continue;
          }
          ...
          addLog(
            `Fila ${i + 1}: ${referencia} → ACTUALIZADO (${camposAportados.join(
              ", "
            )})`,
            "success"
          );
          ...
          if (!datosExcel.descripcion) {
            addLog(
              `Fila ${
                i + 1
              }: ${referencia} → ERROR para crear hay que aportar descripción`,
              "error"
            );
            errores++;
            continue;
          ...
          if (datosExcel.grupodescuento === undefined) {
            addLog(
              `Fila ${
                i + 1
              }: ${referencia} → ERROR para crear hay que aportar grupodescuento`,
              "error"
            );
            errores++;
            continue;
          ...
          addLog(
            `Fila ${i + 1}: ${referencia} → NUEVO (ID ${
              data.id
            }, campos: ${camposAportados.join(", ")})`,
            "success"
          );
          nuevos++;
        } else {
    ...
    addLog(
      `  • Productos nuevos creados:     ${nuevos}`,
      nuevos > 0 ? "success" : "info"
    );
    addLog(
      `  • Errores:                      ${errores}`,
    ...
    setStatus &&
      setStatus(
        `Tarifa procesada: ${actualizados} actualizados, ${nuevos} nuevos, ${errores} errores${modo}`,
  ...
  return (
    <div>
      ...
      >
        Carga un fichero Excel de tarifa con productos. Para cada fila se
        comprueba si el producto existe: si existe se actualiza su PVP, si no
        existe se da de alta. La primera fila debe contener los nombres de
        columna. Columnas obligatorias: <strong>Referencia</strong>,{" "}
        <strong>Descripción</strong>, y o bien <strong>SubFamilia</strong> o
        bien <strong>GruposDescuento</strong> (si no hay subfamilia, se busca la
        subfamilia que contiene ese grupo de descuento).
      </p>
      ...
      <div style={{ marginBottom: 12 }}>
        <div
          ...
          style={{
            padding: "20px",
            border: `2px dashed ${dragOver ? "#2563eb" : "#d4d4d4"}`,
            borderRadius: 8,
        ...
        >
          ...
          <div style={{ marginTop: 6 }}>
            {dragOver ? (
              ...
              <>
                Arrastra un Excel/CSV (.xlsx, .xls, .csv), o haz click para
                elegir uno.
              </>
            )}
          </div>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        >
          <label style={{ fontSize: 12, color: "#171717", fontWeight: 500 }}>
            Si existe el producto, sobreescribir:
          </label>
          <div
            style={{
              display: "inline-flex",
          ...
          >
            ...
            <button
              onClick={() => setSobreescribir(false)}
              style={{
                padding: "4px 14px",
                fontSize: 11,
                border: "none",
        ...
        >
          ...
          <ol style={{ margin: "4px 0 0 18px", padding: 0 }}>
            <li>
              Carga una Excel con{" "}
              <code>referencia, pvp, descripcion, grupodescuento, nombre</code>{" "}
              y el selector{" "}
              <strong>"Si existe el producto, sobreescribir: NO"</strong>. Esto
              creará los productos nuevos con toda la información y dejará los
              existentes intactos.
            </li>
            <li>
              Vuelve a pasar la misma Excel (o solo con{" "}
              <code>referencia, pvp, grupodescuento</code>) y el selector{" "}
              <strong>"Si existe el producto, sobreescribir: SÍ"</strong>. Esto
              actualizará los PVP y los grupos descuento de los productos
              existentes sin sobreescribir nombre ni descripción.
            </li>
          </ol>
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <span style={{ color: "#64748b", fontWeight: 400 }}>
              ({fileData.rows.length} filas en "{fileData.origen}")
            </span>
            :
          </div>
            ...
            {fileData.headers.map((h, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "3px 8px",
                  borderRadius: 4,
                  background: mapping[i] ? "#dcfce7" : "#fee2e2",
                  border: mapping[i]
                    ? "1px solid #86efac"
                    : "1px solid #fca5a5",
                  color: mapping[i] ? "#14532d" : "#991b1b",
                }}
              >
                <strong>{h}</strong>{" "}
                {mapping[i] ? "→ " + mapping[i] : "(no reconocida)"}
              </div>
            ))}
          </div>
      ...
      {fileData && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <button
            ...
            disabled={procesando || erroresMapeo.length > 0}
            style={{
              padding: "7px 18px",
              borderRadius: 6,
              border:
                !procesando && erroresMapeo.length === 0
                  ? "1px solid #16a34a"
                  : "1px solid #d4d4d4",
              background:
                !procesando && erroresMapeo.length === 0
          ...
          >
            <BtnContent
              icon={procesando ? RefreshCw : Check}
              iconColor={
                !procesando && erroresMapeo.length === 0 ? "#14532d" : "#737373"
              }
            >
              {procesando ? "Procesando..." : "Aplicar tarifa a BD"}
            </BtnContent>
          </button>
          {procesando && (
            <button
              onClick={() => {
                ...
                setStatus &&
                  setStatus("Solicitando parada del procesado...", "working");
              }}
              style={{
                padding: "7px 18px",
                borderRadius: 6,
            ...
            >
              <BtnContent icon={X} iconColor="#dc2626">
                Parar
              </BtnContent>
            </button>
          )}
      ...
      {resumen && (
        ...
        >
          <strong>Resumen:</strong> {resumen.total} filas procesadas —{" "}
          <span style={{ color: "#16a34a" }}>
            {resumen.actualizados} actualizados
          </span>
          , <span style={{ color: "#2563eb" }}>{resumen.nuevos} nuevos</span>,{" "}
          <span style={{ color: "#dc2626" }}>{resumen.errores} errores</span>,{" "}
          <span style={{ color: "#d97706" }}>{resumen.avisos} avisos</span>
        </div>
      )}
      ...
      <div style={{ marginTop: 8 }}>
        ...
        >
          <span>
            Progreso ({log.length} entrada{log.length !== 1 ? "s" : ""})
          </span>
          {log.length > 0 && (
            <div style={{ display: "inline-flex", gap: 6 }}>
                    ...
                    navigator.clipboard
                      ...
                      .then(
                        () =>
                          setStatus &&
                          setStatus(
                            `Log (${log.length} líneas) copiado al portapapeles`,
                            "success"
                          )
                      )
                      .catch(
              ...
              >
                <BtnContent icon={Copy} iconColor="#475569">
                  Copiar log
                </BtnContent>
              </button>
              <button
              ...
              >
                <BtnContent icon={Trash2} iconColor="#475569">
                  Limpiar log
                </BtnContent>
              </button>
            </div>
        ...
        >
          {log.length === 0 ? (
            ...
            >
              Sin actividad. Carga un fichero Excel y pulsa "Aplicar tarifa a
              BD" para ver el progreso aquí.
            </div>
          ) : (
            log.map((l, i) => (
              <div
...
function UsuariosSection({ setStatus }) {
      ...
      setStatus &&
        setStatus(
          `Usuario "${draft.usuario}" ${
            seleccionado === "nuevo" ? "creado" : "actualizado"
          }`,
          "success"
        );
      setSeleccionado(null);
  ...
  return (
    <div>
      ...
      >
        Crear, editar y borrar usuarios de la aplicación. Cada usuario tiene su
        propio código de presupuestos y permisos para crear, editar o borrar
        presupuestos, clientes y productos.
      </p>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <BtnContent icon={Plus} iconColor="#14532d">
                Nuevo
              </BtnContent>
            </button>
            <button
                  ...
                  {usuariosFiltrados.map((u) => (
                    <tr
                      ...
                      style={{
                        ...
                        background:
                          seleccionado === u.id ? "#eff6ff" : "transparent",
                      }}
                    >
                      <td
                        style={{
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setConfirmBorrar({ id: u.id, usuario: u.usuario })
                          }
                          title="Borrar"
                          style={{
                            padding: "3px 8px",
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <input
                  value={draft.codigopresupuestos || ""}
                  onChange={(e) =>
                    setDraft({ ...draft, codigopresupuestos: e.target.value })
                  }
                  maxLength={10}
                  placeholder="ej. CDM"
                  style={{
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                <div>
                  ...
                  <Toggle
                    label="Crear"
                    checked={draft.permisocrearpresupuesto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisocrearpresupuesto: v })
                    }
                  />
                  <Toggle
                    label="Editar"
                    checked={draft.permisoeditarpresupuesto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoeditarpresupuesto: v })
                    }
                  />
                  <Toggle
                    label="Borrar"
                    checked={draft.permisoborrarpresupuesto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoborrarpresupuesto: v })
                    }
                  />
                </div>
                <div>
                  ...
                  <Toggle
                    label="Crear"
                    checked={draft.permisocrearcliente}
                    onChange={(v) =>
                      setDraft({ ...draft, permisocrearcliente: v })
                    }
                  />
                  <Toggle
                    label="Editar"
                    checked={draft.permisoeditarcliente}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoeditarcliente: v })
                    }
                  />
                  <Toggle
                    label="Borrar"
                    checked={draft.permisoborrarcliente}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoborrarcliente: v })
                    }
                  />
                </div>
                <div>
                  ...
                  <Toggle
                    label="Crear"
                    checked={draft.permisocrearproducto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisocrearproducto: v })
                    }
                  />
                  <Toggle
                    label="Editar"
                    checked={draft.permisoeditarproducto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoeditarproducto: v })
                    }
                  />
                  <Toggle
                    label="Borrar"
                    checked={draft.permisoborrarproducto}
                    onChange={(v) =>
                      setDraft({ ...draft, permisoborrarproducto: v })
                    }
                  />
                </div>
              </div>
            ...
            >
              ...
              >
                <BtnContent icon={Save} iconColor="#14532d">
                  {seleccionado === "nuevo" ? "Crear" : "Guardar"}
                </BtnContent>
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => borrar(confirmBorrar.id, confirmBorrar.usuario)}
                style={{
                  padding: "6px 14px",
                  borderRadius: 6,
                  border: "1px solid #dc2626",
...
function MantenimientoSubfamiliaDialog({ onClose, setStatus }) {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setStatus &&
        setStatus("Error cargando subfamilias: " + e.message, "error");
    } finally {
      setCargando(false);
    }
  };
  ...
  const filtradas = subfamilias.filter(
    (sf) =>
      !busqueda.trim() ||
      String(sf.subfamilia || "")
        .toLowerCase()
        .includes(busqueda.toLowerCase()) ||
      String(sf.descripcion || "")
        .toLowerCase()
        .includes(busqueda.toLowerCase()) ||
      String(sf.familia || "")
        .toLowerCase()
  ...
  const guardarEdicion = async () => {
    ...
    if (!editar.subfamilia || !editar.subfamilia.trim()) {
      setStatus &&
        setStatus("El nombre de la subfamilia no puede estar vacío", "error");
      return;
    }
    setGuardando(true);
    try {
      ...
      setStatus &&
        setStatus(`Subfamilia "${editar.subfamilia}" actualizada`, "success");
      setEditar(null);
      cargar();
    } catch (e) {
      setStatus && setStatus("Error guardando: " + e.message, "error");
  ...
  const comprobarYBorrar = async () => {
    ...
    setStatus &&
      setStatus(
        `Comprobando referencias de "${seleccionada.subfamilia}"...`,
        "working"
      );
    try {
      ...
      if (!data.borrable) {
        setStatus &&
          setStatus(
            `No se puede borrar "${seleccionada.subfamilia}": está referenciada en ${data.en_gruposdescuento} grupo(s) de descuento.`,
            "error"
          );
        return;
      }
      setStatus &&
        setStatus("Subfamilia sin referencias, listo para borrar", "info");
      setConfirmBorrar({
        id: seleccionada.id,
        subfamilia: seleccionada.subfamilia,
      });
  ...
  const borrarConfirmado = async () => {
    ...
    setBorrando(true);
    try {
      const res = await fetch(`${API_URL}/subfamilias/${confirmBorrar.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
      ...
      setStatus &&
        setStatus(
          `Subfamilia "${confirmBorrar.subfamilia}" borrada`,
          "success"
        );
      setConfirmBorrar(null);
  ...
  return (
    <div
      ...
      onClick={() => !editar && !borrando && !guardando && onClose()}
    >
      <div
        style={{
          background: "#fff",
      ...
      >
        ...
        >
          ...
          >
            <Icon as={Wrench} size={18} color="#1e3a5f" /> Mantenimiento BD
            Subfamilia
          </h2>
          <button
            onClick={onClose}
        ...
        >
          ...
          <span style={{ fontSize: 11, color: "#737373" }}>
            {filtradas.length} subfamilia{filtradas.length !== 1 ? "s" : ""}
          </span>
          <button
            onClick={cargar}
            title="Recargar"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "5px 10px", color: "#525252" }}>
                        {editando ? (
                          <select
                            value={editar.idfamiliarelacionada || ""}
                            onChange={(e) =>
                              setEditar({
                                ...editar,
                                idfamiliarelacionada: e.target.value
        ...
        >
          <div style={{ display: "flex", gap: 6 }}>
            <button
              onClick={comprobarYBorrar}
              disabled={!seleccionada || comprobandoUso || !!editar}
              style={{
                padding: "7px 14px",
                borderRadius: 6,
                border:
                  seleccionada && !editar
                    ? "1px solid #fca5a5"
                    : "1px solid #d4d4d4",
                background: seleccionada && !editar ? "#fef2f2" : "#f5f5f5",
                color: seleccionada && !editar ? "#991b1b" : "#737373",
                cursor:
                  seleccionada && !editar && !comprobandoUso
                    ? "pointer"
                    : "default",
                fontSize: 12,
              }}
            >
              <BtnContent
                icon={comprobandoUso ? RefreshCw : Trash2}
                iconColor={seleccionada && !editar ? "#dc2626" : "#737373"}
              >
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {!editar ? (
              <button
                ...
                style={{
                  ...
                  border: seleccionada
                    ? "1px solid #cbd5e1"
                    : "1px solid #d4d4d4",
                  background: seleccionada ? "#fff" : "#f5f5f5",
                  color: seleccionada ? "#171717" : "#737373",
                  cursor: seleccionada ? "pointer" : "default",
                  fontSize: 12,
                }}
              >
                <BtnContent
                  icon={Edit3}
                  iconColor={seleccionada ? "#475569" : "#737373"}
                >
                  Editar
                </BtnContent>
              </button>
              ...
              <>
                ...
                >
                  <BtnContent
                    icon={guardando ? RefreshCw : Check}
                    iconColor="#14532d"
                  >
        ...
        {confirmBorrar && (
          <div
            ...
            onClick={() => !borrando && setConfirmBorrar(null)}
          >
            <div
              style={{
                background: "#fff",
            ...
            >
              ...
              >
                <Icon as={Trash2} size={18} color="#dc2626" /> ¿Borrar
                subfamilia?
              </h3>
              <p style={{ fontSize: 13, color: "#525252", marginBottom: 12 }}>
                Vas a borrar la subfamilia{" "}
                <strong>"{confirmBorrar.subfamilia}"</strong>.
              </p>
              <div
                style={{
                  background: "#fef2f2",
...
function MantenimientoFamiliaDialog({ onClose, setStatus }) {
  ...
  const guardarEdicion = async () => {
    ...
    if (!editar.familia || !editar.familia.trim()) {
      setStatus &&
        setStatus("El nombre de la familia no puede estar vacío", "error");
      return;
    }
    setGuardando(true);
    try {
      ...
      setStatus &&
        setStatus(`Familia "${editar.familia}" actualizada`, "success");
      setEditar(null);
      setSeleccionada({
        ...seleccionada,
        familia: editar.familia.trim(),
  ...
  const comprobarYBorrar = async () => {
    ...
    setStatus &&
      setStatus(
        `Comprobando referencias de "${seleccionada.familia}"...`,
        "working"
      );
    try {
      ...
      if (!data.borrable) {
        setStatus &&
          setStatus(
            `No se puede borrar "${seleccionada.familia}": está referenciada en ${data.en_subfamilias} subfamilia(s).`,
            "error"
          );
        return;
      }
      setStatus &&
        setStatus("Familia sin referencias, listo para borrar", "info");
  ...
  const borrarConfirmado = async () => {
    ...
    setBorrando(true);
    try {
      const res = await fetch(`${API_URL}/familias/${confirmBorrar.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
      ...
      setStatus &&
        setStatus(`Familia "${confirmBorrar.familia}" borrada`, "success");
      setConfirmBorrar(null);
      setSeleccionada(null);
      cargar();
    } catch (e) {
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <span style={{ fontSize: 11, color: "#737373" }}>
            {filtradas.length} familia{filtradas.length !== 1 ? "s" : ""}
          </span>
          <button
            onClick={cargar}
            title="Recargar"
                      ...
                      <td style={{ padding: "5px 10px", color: "#171717" }}>
                        {editando ? (
                          <input
                            ...
                            onChange={(e) =>
                              setEditar({ ...editar, familia: e.target.value })
                            }
                            style={{
                              width: "100%",
                              padding: "3px 6px",
        ...
        {confirmBorrar && (
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: 13, color: "#525252", marginBottom: 12 }}>
                Vas a borrar la familia{" "}
                <strong>"{confirmBorrar.familia}"</strong>.
              </p>
              <div
                style={{
                  background: "#fef2f2",
...
function MantenimientoSection({ setStatus }) {
  ...
  const asignarAddLog = (texto, tipo = "info") => {
    ...
    setTimeout(() => {
      if (asignarLogRef.current)
        asignarLogRef.current.scrollTop = asignarLogRef.current.scrollHeight;
    }, 0);
  };
  ...
  const ejecutarAsignacionGrupos = async () => {
    ...
    try {
      ...
      if (!resGrupos.ok)
        throw new Error("Error cargando grupos: " + resGrupos.status);
      ...
      for (const g of grupos) {
        ...
        const raices = String(g.raizproductos)
          .split(",")
          .map((r) => r.trim())
          .filter(Boolean);
        ...
        for (const raiz of raices) {
          ...
          try {
            const resC = await fetch(
              `${API_URL}/productos/contar-por-raiz?raiz=${encodeURIComponent(
                raiz
              )}`
            );
            if (resC.ok) {
          ...
          if (count === 0) {
            asignarAddLog(
              `${g.grupodescuentospain} / raíz "${raiz}" → 0 productos coinciden, se omite`,
              "info"
            );
            continue;
          }
          ...
          if (decision === "skip") {
            asignarAddLog(
              `${g.grupodescuentospain} / raíz "${raiz}" (${count} productos) → CANCELADO por el usuario`,
              "warning"
            );
            canceladas++;
            continue;
          ...
          try {
            const resApl = await fetch(
              `${API_URL}/productos/aplicar-grupo-por-raiz`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            asignarAddLog(
              `${g.grupodescuentospain} / raíz "${raiz}" → ${d.actualizados} productos actualizados`,
              "success"
            );
            totalActualizados += d.actualizados || 0;
          } catch (err) {
            asignarAddLog(
              `${g.grupodescuentospain} / raíz "${raiz}" → ERROR: ${err.message}`,
              "error"
            );
          }
        }
      ...
      asignarAddLog(
        `Terminado: ${combinaciones} combinaciones (${confirmadas} aplicadas, ${canceladas} canceladas). Total productos actualizados: ${totalActualizados}`,
        "success"
      );
      setStatus &&
        setStatus(
          `Asignación completada: ${totalActualizados} productos actualizados`,
          "success"
        );
    } catch (e) {
  ...
  const gdAddLog = (texto, tipo = "info") => {
    ...
    setTimeout(() => {
      if (gdLogRef.current)
        gdLogRef.current.scrollTop = gdLogRef.current.scrollHeight;
    }, 0);
  };
  ...
  const GD_COLS = [
    {
      key: "grupodescuentospain",
      aliases: [
        "grupodescuentospain",
        "grupo descuento spain",
        "gd spain",
        "gdspain",
        "grupo descuento españa",
        "gd",
      ],
    },
    {
      key: "grupodescuentocasamatriz",
      aliases: [
        "grupodescuentocasamatriz",
        "grupo descuento casa matriz",
        "gd casa matriz",
        "gdcasamatriz",
        "casa matriz",
      ],
  ...
  const cargarGdFile = (file) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!aoa || aoa.length < 2) {
          gdAddLog("Fichero vacío o sin datos", "error");
          return;
        }
        const headers = aoa[0].map((h) => String(h || "").trim());
        const rows = aoa
        ...
        gdAddLog(
          `Excel cargado: ${
            rows.length
          } filas. Columnas detectadas: ${detectadas.join(", ")}`,
          "success"
        );
        ...
        if (!detectadas.includes("subfamilia")) {
          gdAddLog(
            "ℹ Sin columna 'subfamilia': solo se podrán actualizar grupos existentes, no crear nuevos",
            "warning"
          );
        }
      } catch (err) {
  ...
  const aplicarGd = async () => {
    ...
    const idxPorClave = {};
    Object.entries(gdFileData.mapping).forEach(([idx, clave]) => {
      idxPorClave[clave] = parseInt(idx);
    });
    const columnasDetectadas = Object.keys(idxPorClave);

    if (!columnasDetectadas.includes("grupodescuentospain")) {
      gdAddLog(
        "ERROR: el fichero debe tener una columna 'grupodescuentospain'. Se cancela el proceso.",
        "error"
      );
    ...
    if (columnasDetectadas.length < 2) {
      gdAddLog(
        "ERROR: el fichero debe tener al menos 2 columnas (grupodescuentospain + otra). Se cancela el proceso.",
        "error"
      );
      setStatus &&
        setStatus("El fichero debe tener al menos 2 columnas", "error");
    ...
    gdAddLog(
      `Procesando fichero "${gdFileData.nombreFichero || "(desconocido)"}"`,
      "info"
    );
    gdAddLog(
      `Iniciando: ${
        gdFileData.rows.length
      } filas. Columnas detectadas: ${columnasDetectadas.join(", ")}`,
      "info"
    );
    ...
    if (idxPorClave.subfamilia !== undefined) {
      try {
        ...
        gdAddLog(
          `Cargadas ${subfamilias.length} subfamilias de BD para resolver ids`,
          "info"
        );
      } catch (e) {
        gdAddLog(
          "AVISO: no se pudieron cargar subfamilias de BD: " + e.message,
          "warning"
        );
      }
    }
    ...
    for (let i = 0; i < gdFileData.rows.length; i++) {
      ...
      const codigoGD = String(
        row[idxPorClave.grupodescuentospain] ?? ""
      ).trim();
      if (!codigoGD) {
        gdAddLog(`Fila ${i + 1}: sin grupodescuentospain, se omite`, "warning");
        errores++;
        continue;
      }
      ...
      if (idxPorClave.subfamilia !== undefined) {
        ...
        if (subfamTxt) {
          const sf = subfamilias.find(
            (s) =>
              s.subfamilia &&
              s.subfamilia.toUpperCase() === subfamTxt.toUpperCase()
          );
          if (!sf) {
            gdAddLog(
              `Fila ${
                i + 1
              }: ${codigoGD} → ERROR subfamilia "${subfamTxt}" no existe en BD`,
              "error"
            );
            errores++;
            continue;
      ...
      if (idxPorClave.grupodescuentocasamatriz !== undefined) {
        const v = String(
          row[idxPorClave.grupodescuentocasamatriz] ?? ""
        ).trim();
        if (v) {
          payload.grupodescuentocasamatriz = v;
          aportados.push("grupodescuentocasamatriz");
        }
      }
      if (idxPorClave.descripcion !== undefined) {
        const v = String(row[idxPorClave.descripcion] ?? "").trim();
        if (v) {
          payload.descripcion = v;
          aportados.push("descripcion");
        }
      }
      if (idxPorClave.dtoreferencia !== undefined) {
        const raw = String(row[idxPorClave.dtoreferencia] ?? "")
          .trim()
          .replace(",", ".");
        const num = parseFloat(raw);
        if (!isNaN(num)) {
          payload.dtoreferencia = num;
          aportados.push("dtoreferencia");
        }
      }
      if (idxPorClave.raizproductos !== undefined) {
        const v = String(row[idxPorClave.raizproductos] ?? "").trim();
        if (v) {
          payload.raizproductos = v;
          aportados.push("raizproductos");
        }
      }
      if (idxPorClave.dgl1 !== undefined) {
        const raw = String(row[idxPorClave.dgl1] ?? "")
          .trim()
          .replace(",", ".");
        const num = parseFloat(raw);
        if (!isNaN(num)) {
      ...
      if (idxPorClave.dgl2 !== undefined) {
        const raw = String(row[idxPorClave.dgl2] ?? "")
          .trim()
          .replace(",", ".");
        const num = parseFloat(raw);
        if (!isNaN(num)) {
      ...
      try {
        // Buscar si existe
        const resBuscar = await fetch(
          `${API_URL}/gruposdescuento/${encodeURIComponent(codigoGD)}`
        );
        if (resBuscar.status === 404) {
          ...
          if (idSubfamRel === null) {
            gdAddLog(
              `Fila ${
                i + 1
              }: ${codigoGD} → ERROR para crear hay que aportar columna 'subfamilia'`,
              "error"
            );
            errores++;
            continue;
          ...
          gdAddLog(
            `Fila ${i + 1}: ${codigoGD} → ALTA (campos: ${aportados.join(
              ", "
            )})`,
            "success"
          );
          ...
          if (!gdSobreescribir) {
            gdAddLog(
              `Fila ${
                i + 1
              }: ${codigoGD} → ya existe, se omite (sobreescribir = NO)`,
              "info"
            );
            saltados++;
            continue;
          ...
          if (aportados.length === 0) {
            gdAddLog(
              `Fila ${
                i + 1
              }: ${codigoGD} → solo se aportó código, nada que actualizar`,
              "info"
            );
            continue;
          }
          ...
          const qs = qsAportados
            .map((c) => `solo_aportados=${encodeURIComponent(c)}`)
            .join("&");
          const res = await fetch(
            `${API_URL}/gruposdescuento/${existente.id}?${qs}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(body),
          ...
          gdAddLog(
            `Fila ${i + 1}: ${codigoGD} → ACTUALIZADO (campos: ${aportados.join(
              ", "
            )})`,
            "success"
          );
    ...
    gdAddLog(
      `Terminado: ${nuevos} nuevos, ${actualizados} actualizados, ${saltados} omitidos, ${errores} errores`,
      "success"
    );
    setStatus &&
      setStatus(
        `Grupos descuento procesados: ${
          nuevos + actualizados
        } OK, ${errores} errores`,
        errores > 0 ? "error" : "success"
      );
  ...
  const actualizarMasusado = async () => {
    ...
    setStatus &&
      setStatus("Recalculando campo masusado de los productos...", "working");
    try {
      const res = await fetch(`${API_URL}/productos/actualizar-masusado`, {
        method: "POST",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
      ...
      setStatus &&
        setStatus(
          `masusado recalculado: ${data.productos_actualizados} productos actualizados de ${data.referencias_procesadas} referencias procesadas`,
          "success"
        );
    } catch (e) {
      setStatus &&
        setStatus("Error recalculando masusado: " + e.message, "error");
    } finally {
  ...
  const borrarLogsAnteriores = async () => {
    setBorrando(true);
    setStatus &&
      setStatus(`Borrando logs anteriores a ${fechaCorte}...`, "working");
    try {
      const res = await fetch(`${API_URL}/logs/?antes_de=${fechaCorte}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Error " + res.status);
      const data = await res.json();
  ...
  const exportarSubfamilias = async () => {
    ...
    setStatus &&
      setStatus("Generando Excel de Familia/SubFamilia...", "working");
    try {
      const res = await fetch(`${API_URL}/subfamilias/`);
      if (!res.ok) throw new Error("Error " + res.status);
      ...
      setStatus &&
        setStatus(
          `Excel generado con ${subfamilias.length} subfamilias`,
          "success"
        );
    } catch (e) {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <Icon as={FileText} size={16} color="#2563eb" /> Mantenimiento tabla
          Log
        </h3>
        <p
          style={{
        ...
        >
          Consulta el histórico de eventos de la aplicación y borra los
          registros antiguos para liberar espacio en la base de datos.
        </p>
        <button
          onClick={abrirLogs}
          style={{
        ...
        >
          <BtnContent icon={FileText} iconColor="#2563eb">
            Ver tabla de logs
          </BtnContent>
        </button>
      </div>
      ...
      >
        ...
        >
          <Icon as={Package} size={16} color="#2563eb" /> Mantenimiento tabla
          Productos
        </h3>
        <p
          style={{
        ...
        >
          Actualiza el campo <strong>masusado</strong> de cada producto contando
          cuántas veces aparece su referencia en la tabla{" "}
          <code>detallepresupuestos</code>. Solo se incrementa el valor si el
          nuevo número es mayor que el actual (nunca se disminuye, así no se
          pierde el histórico si se han borrado presupuestos antiguos).
        </p>
        <button
          onClick={actualizarMasusado}
          disabled={actualizandoMasusado}
          style={{
            padding: "7px 16px",
            borderRadius: 6,
            border: "1px solid #2563eb",
            background: "#eff6ff",
            color: "#1e40af",
            cursor: actualizandoMasusado ? "default" : "pointer",
            fontSize: 12,
            fontWeight: 600,
          }}
        >
          <BtnContent
            icon={actualizandoMasusado ? RefreshCw : TrendingUp}
            iconColor="#2563eb"
          >
            {actualizandoMasusado
              ? "Calculando..."
              : "Recalcular campo masusado"}
          </BtnContent>
        </button>
        {ultimoResultadoMasusado && (
          ...
          >
            ...
            <ul style={{ margin: "4px 0 0 16px", padding: 0, lineHeight: 1.6 }}>
              <li>
                Referencias distintas procesadas:{" "}
                <strong>
                  {ultimoResultadoMasusado.referencias_procesadas}
                </strong>
              </li>
              <li>
                Productos actualizados (nuevo valor mayor):{" "}
                <strong style={{ color: "#15803d" }}>
                  {ultimoResultadoMasusado.productos_actualizados}
                </strong>
              </li>
              <li>
                Sin cambio (valor actual ya era igual o mayor):{" "}
                <strong>{ultimoResultadoMasusado.sin_cambio}</strong>
              </li>
              <li>
                Referencias sin producto en BD:{" "}
                <strong style={{ color: "#d97706" }}>
                  {ultimoResultadoMasusado.referencias_sin_producto}
                </strong>
              </li>
            </ul>
          </div>
      ...
      >
        ...
        >
          <Icon as={Layers} size={16} color="#16a34a" /> Mantenimiento tablas
          Familia / SubFamilia
        </h3>
        <p
          style={{
        ...
        >
          Genera un Excel con el listado completo de subfamilias indicando a qué
          familia pertenecen y su descuento. Útil para revisar la estructura de
          productos.
        </p>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button
            onClick={exportarSubfamilias}
            disabled={exportandoSubfam}
            style={{
              padding: "7px 16px",
              borderRadius: 6,
              border: "1px solid #16a34a",
              background: "#f0fdf4",
              color: "#14532d",
              cursor: exportandoSubfam ? "default" : "pointer",
              fontSize: 12,
              fontWeight: 600,
            }}
          >
            <BtnContent
              icon={exportandoSubfam ? RefreshCw : Download}
              iconColor="#16a34a"
            >
              {exportandoSubfam ? "Generando..." : "Exportar listado a Excel"}
            </BtnContent>
          </button>
          <button
            onClick={() => setShowFamiliasDialog(true)}
            style={{
              padding: "7px 16px",
              borderRadius: 6,
              border: "1px solid #cbd5e1",
          ...
          >
            <BtnContent icon={Wrench} iconColor="#475569">
              Mantenimiento BD Familia
            </BtnContent>
          </button>
          <button
            onClick={() => setShowSubfamiliasDialog(true)}
            style={{
              padding: "7px 16px",
              borderRadius: 6,
              border: "1px solid #cbd5e1",
          ...
          >
            <BtnContent icon={Wrench} iconColor="#475569">
              Mantenimiento BD Subfamilia
            </BtnContent>
          </button>
        </div>
        {showFamiliasDialog && (
          <MantenimientoFamiliaDialog
            onClose={() => setShowFamiliasDialog(false)}
            setStatus={setStatus}
          />
        )}
        {showSubfamiliasDialog && (
          <MantenimientoSubfamiliaDialog
            onClose={() => setShowSubfamiliasDialog(false)}
            setStatus={setStatus}
          />
        )}
      </div>
      ...
      >
        ...
        >
          <Icon as={Percent} size={16} color="#7c3aed" /> Mantenimiento tabla
          Grupos Descuento
        </h3>
        <p
          style={{
        ...
        >
          Carga un Excel para crear o actualizar grupos de descuento. La primera
          fila debe tener los nombres de los campos. Obligatorios:{" "}
          <strong>grupodescuentospain</strong>. Para <em>crear nuevos</em>{" "}
          grupos, también es necesaria la columna <strong>subfamilia</strong>.
          Opcionales: <code>grupodescuentocasamatriz</code>,{" "}
          <code>descripcion</code>, <code>dtoreferencia</code>,{" "}
          <code>raizproductos</code>, <code>dgl1</code>, <code>dgl2</code>,{" "}
          <code>dgl1</code>, <code>dgl2</code>. Las celdas vacías del Excel NO
          sobreescriben valores existentes.
        </p>
        ...
        >
          <input
            ...
            style={{ display: "none" }}
            onChange={(e) => {
              if (e.target.files?.[0]) cargarGdFile(e.target.files[0]);
              e.target.value = "";
            }}
          />
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: "#525252" }}>
            {gdFileData ? (
              <>
                <strong>{gdFileData.rows.length} filas</strong> cargadas. Pulsa
                el botón "Aplicar" para procesar.
              </>
            ) : (
          ...
          {gdFileData && (
            <div style={{ marginTop: 6, fontSize: 11, color: "#737373" }}>
              Columnas detectadas:{" "}
              {Object.values(gdFileData.mapping).join(", ") || "ninguna"}
            </div>
          )}
        </div>
        ...
        {gdFileData &&
          (() => {
            ...
            if (!detectadas.includes("grupodescuentospain")) {
              erroresMapeoGD.push(
                "Falta columna 'grupodescuentospain' (obligatoria)"
              );
            }
            // Al menos 2 columnas reconocidas
            if (detectadas.length < 2) {
              erroresMapeoGD.push(
                "El fichero debe tener al menos 2 columnas reconocidas (grupodescuentospain + otra)"
              );
            }
            return (
              <div
              ...
              >
                ...
                >
                  Mapeo de columnas detectado{" "}
                  <span style={{ color: "#64748b", fontWeight: 400 }}>
                    ({gdFileData.rows.length} filas en "
                    {gdFileData.nombreFichero}")
                  </span>
                  :
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {gdFileData.headers.map((h, i) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: "3px 8px",
                        borderRadius: 4,
                        background: gdFileData.mapping[i]
                          ? "#dcfce7"
                          : "#fee2e2",
                        border: gdFileData.mapping[i]
                          ? "1px solid #86efac"
                          : "1px solid #fca5a5",
                        color: gdFileData.mapping[i] ? "#14532d" : "#991b1b",
                      }}
                    >
                      <strong>{h}</strong>{" "}
                      {gdFileData.mapping[i]
                        ? "→ " + gdFileData.mapping[i]
                        : "(no reconocida)"}
                    </div>
                  ))}
        ...
        >
          <label style={{ fontSize: 12, color: "#171717", fontWeight: 500 }}>
            Si el Grupo de descuento existe, sobreescribir:
          </label>
          <div
            style={{
              display: "inline-flex",
          ...
          >
            <button
              onClick={() => setGdSobreescribir(true)}
              style={{
                padding: "4px 14px",
                fontSize: 11,
                border: "none",
            ...
            <button
              onClick={() => setGdSobreescribir(false)}
              style={{
                padding: "4px 14px",
                fontSize: 11,
                border: "none",
          ...
          {gdFileData && (
            <>
              <button
                ...
                style={{
                  ...
                  border: gdProcesando
                    ? "1px solid #d4d4d4"
                    : "1px solid #16a34a",
                  background: gdProcesando ? "#f5f5f5" : "#dcfce7",
                  color: gdProcesando ? "#737373" : "#14532d",
                  cursor: gdProcesando ? "default" : "pointer",
                  fontSize: 12,
                  fontWeight: 600,
                }}
              >
                <BtnContent
                  icon={gdProcesando ? RefreshCw : Check}
                  iconColor={gdProcesando ? "#737373" : "#14532d"}
                >
              ...
              {gdProcesando && (
                <button
                  onClick={() => {
                    gdStopRef.current = true;
                    setStatus && setStatus("Solicitando parada...", "working");
                  }}
                  style={{
                    padding: "7px 18px",
                    borderRadius: 6,
              ...
              >
                <BtnContent icon={X} iconColor="#475569">
                  Limpiar fichero
                </BtnContent>
              </button>
            </>
        ...
        <div>
          ...
          >
            <span>
              Progreso ({gdLog.length} entrada{gdLog.length !== 1 ? "s" : ""})
            </span>
            {gdLog.length > 0 && (
              <div style={{ display: "inline-flex", gap: 6 }}>
                <button
                      ...
                      navigator.clipboard
                        ...
                        .then(
                          () =>
                            setStatus &&
                            setStatus(
                              `Log (${gdLog.length} líneas) copiado al portapapeles`,
                              "success"
                            )
                        )
                        .catch(
                          () =>
                            setStatus && setStatus("No se pudo copiar", "error")
                        );
                    }
                  }}
                  style={{
          ...
          >
            {gdLog.length === 0 ? (
              ...
              >
                Sin actividad. Carga un Excel y pulsa "Aplicar a BD" para ver el
                progreso.
              </div>
            ) : (
              gdLog.map((l, i) => (
        ...
        >
          ...
          >
            <Icon as={Percent} size={14} color="#7c3aed" /> Asignar grupo
            descuento por raíz
          </h4>
          <p
            style={{
          ...
          >
            Para cada grupo de descuento, lee el campo{" "}
            <code>raizproductos</code> (raíces separadas por comas) y asigna ese
            grupo a todos los productos cuya referencia empiece por cada raíz.
            Se sobreescribe el grupo previo del producto si tenía uno asignado.{" "}
            <strong>
              Se pedirá confirmación por cada combinación grupo + raíz.
            </strong>
          </p>
          <div
            style={{
          ...
          >
            <button
              onClick={ejecutarAsignacionGrupos}
              disabled={asignarProcesando}
              style={{
                padding: "7px 16px",
                borderRadius: 6,
                border: asignarProcesando
                  ? "1px solid #d4d4d4"
                  : "1px solid #7c3aed",
                background: asignarProcesando ? "#f5f5f5" : "#f5f3ff",
                color: asignarProcesando ? "#737373" : "#5b21b6",
                cursor: asignarProcesando ? "default" : "pointer",
                fontSize: 12,
                fontWeight: 600,
              }}
            >
              <BtnContent
                icon={asignarProcesando ? RefreshCw : Percent}
                iconColor={asignarProcesando ? "#737373" : "#7c3aed"}
              >
                {asignarProcesando
                  ? "Procesando..."
                  : "Asignar grupos descuento por raíz"}
              </BtnContent>
            </button>
            {asignarProcesando && (
              <button
                onClick={() => {
                  asignarStopRef.current = true;
                  setStatus && setStatus("Solicitando parada...", "working");
                }}
                style={{
                  padding: "7px 16px",
          ...
          >
            <span>
              Progreso ({asignarLog.length} entrada
              {asignarLog.length !== 1 ? "s" : ""})
            </span>
            {asignarLog.length > 0 && (
              <div style={{ display: "inline-flex", gap: 6 }}>
                <button
                  onClick={() => {
                    const texto = asignarLog
                      .map((l) => {
                        const prefijo =
                          l.tipo === "error"
                            ? "[ERROR]"
                      ...
                      navigator.clipboard
                        ...
                        .then(
                          () =>
                            setStatus &&
                            setStatus(
                              `Log (${asignarLog.length} líneas) copiado al portapapeles`,
                              "success"
                            )
                        )
                        .catch(
          ...
          >
            {asignarLog.length === 0 ? (
              ...
              >
                Sin actividad. Pulsa "Asignar grupos descuento por raíz" para
                empezar.
              </div>
            ) : (
              asignarLog.map((l, i) => (
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <Icon as={Percent} size={16} color="#7c3aed" /> Confirmar
                asignación
              </h3>
            </div>
            <p
            ...
            >
              ¿Asignar el grupo descuento{" "}
              <strong style={{ color: "#7c3aed" }}>{confirmRaiz.codigo}</strong>{" "}
              a los productos cuya referencia empiece por{" "}
              <code
                style={{
                  background: "#f5f3ff",
                  padding: "1px 6px",
            ...
            <p style={{ fontSize: 12, color: "#525252", marginBottom: 16 }}>
              Se actualizarán{" "}
              <strong style={{ color: "#16a34a" }}>{confirmRaiz.count}</strong>{" "}
              producto{confirmRaiz.count !== 1 ? "s" : ""}. Si tenían un grupo
              previo, será sobreescrito.
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
              <button
                onClick={() => confirmRaiz.resolve("cancel_all")}
                style={{
                  padding: "6px 14px",
                  borderRadius: 6,
                  border: "1px solid #fca5a5",
              ...
              <button
                onClick={() => confirmRaiz.resolve("ok")}
                style={{
                  padding: "6px 18px",
                  borderRadius: 6,
                  border: "1px solid #16a34a",
              ...
              >
                <BtnContent icon={Check} iconColor="#14532d">
                  Aceptar y aplicar
                </BtnContent>
              </button>
            </div>
          ...
          >
            ...
            >
              <span style={{ fontSize: 12, color: "#64748b" }}>
                Total:{" "}
                <strong style={{ color: "#1e3a5f" }}>{logs.length}</strong>{" "}
                registros
              </span>
              <button
                onClick={cargarLogs}
              ...
              <button
                onClick={() => setConfirmBorrarLogs(true)}
                disabled={!fechaCorte || borrando}
                style={{
                  padding: "6px 14px",
                  fontSize: 12,
                  borderRadius: 6,
                  border: "1px solid #fca5a5",
                  background: fechaCorte ? "#fff" : "#fafafa",
                  color: fechaCorte ? "#dc2626" : "#cbd5e1",
                  cursor: fechaCorte ? "pointer" : "default",
                  fontWeight: 500,
                }}
              >
                <BtnContent
                  icon={Trash2}
                  iconColor={fechaCorte ? "#dc2626" : "#cbd5e1"}
                >
                  Borrar anteriores
                </BtnContent>
              </button>
            ...
            >
              ...
              >
                <BtnContent icon={X} iconColor="#fff">
                  Cerrar
                </BtnContent>
              </button>
            </div>
            ...
            {confirmBorrarLogs && (
              ...
              >
                ...
                >
                  ...
                  >
                    Se borrarán todos los registros de la tabla Log con fecha{" "}
                    <strong>anterior</strong> al{" "}
                  ...
                  >
                    <button
                      onClick={() => setConfirmBorrarLogs(false)}
                      disabled={borrando}
                      style={{
                        padding: "7px 16px",
                        borderRadius: 6,
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          {OPCIONES_MENU.map((opt) => (
            ...
            >
              <Icon
                as={opt.icon}
                size={14}
                color={seccion === opt.id ? "#2563eb" : "#64748b"}
              />
              <span>{opt.label}</span>
            </div>
          ))}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 40px" }}>
          ...
          {seccion === "mantenimiento" && (
            <MantenimientoSection setStatus={setStatus} />
          )}
          ...
          {seccion === "estilos" && (
            <div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button
                    onClick={() => {
                      ...
                      setStatus &&
                        setStatus(
                          "Estilos exportados a fichero JSON",
                          "success"
                        );
                    }}
                  ...
                  >
                    <BtnContent icon={Download} iconColor="#475569">
                      Exportar JSON
                    </BtnContent>
                  </button>
                  <button
                    onClick={() =>
                      document.getElementById("import-estilos-input")?.click()
                    }
                  ...
                  >
                    <BtnContent icon={FileUp} iconColor="#475569">
                      Importar JSON
                    </BtnContent>
                  </button>
                  <input
                    id="import-estilos-input"
                    type="file"
                    ...
                    onChange={(e) => {
                      ...
                      reader.onload = (ev) => {
                        try {
                          ...
                          setEstilosLocal(fusionado);
                          setStatus &&
                            setStatus(
                              `Estilos importados desde "${file.name}". Pulsa "Aplicar y guardar" para aplicar los cambios.`,
                              "success"
                            );
                        } catch (err) {
                          setStatus &&
                            setStatus(
                              "Error importando estilos: " + err.message,
                              "error"
                            );
                        }
                      };
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: hayCambios
                        ? "1px solid #16a34a"
                        : "1px solid #d4d4d4",
                      background: hayCambios ? "#dcfce7" : "#f5f5f5",
                      color: hayCambios ? "#14532d" : "#737373",
                  ...
                  >
                    <BtnContent
                      icon={Save}
                      iconColor={hayCambios ? "#14532d" : "#737373"}
                    >
                      Aplicar y guardar
                    </BtnContent>
                  </button>
              ...
              >
                Personaliza la apariencia de cada tipo de línea (naturaleza).
                Los cambios se guardan en el navegador (localStorage) y se
                aplican al activar "Aplicar Estructura" en el presupuesto.
              </p>
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            border: !cfg.bg ? "1px dashed #cbd5e1" : "none",
                          }}
                        >
                      ...
                      >
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              actualizar(nat.key, "fontFamily", e.target.value)
                            }
                            style={{
                              width: "100%",
                              padding: "4px 6px",
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              value={cfg.bg}
                              onChange={(e) =>
                                actualizar(nat.key, "bg", e.target.value)
                              }
                              style={{
                                width: 36,
                                height: 26,
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              value={cfg.color}
                              onChange={(e) =>
                                actualizar(nat.key, "color", e.target.value)
                              }
                              style={{
                                width: 36,
                                height: 26,
      ...
      {confirmReset && (
        ...
        >
          ...
          >
            ...
            >
              Se restaurarán todos los estilos a sus valores originales y se
              borrarán los cambios guardados.
              <br />
            ...
            >
              ...
              >
                <BtnContent icon={RefreshCw} iconColor="#fff">
                  Sí, restaurar
                </BtnContent>
              </button>
            </div>
      ...
      {statusMessage && (
        ...
        >
          ...
          {statusMessage.timestamp && (
            ...
            >
              {new Date(statusMessage.timestamp).toLocaleTimeString("es-ES", {
                hour: "2-digit",
                minute: "2-digit",
                second: "2-digit",
              })}
...
function TreeNode({ node, activeId, onSelect, depth = 0 }) {
  ...
  const isChildActive =
    hasChildren && node.children.some((c) => c.id === activeId);
  ...
  return (
    <div>
      <div
        ...
        onMouseLeave={(e) => {
          if (!isActive)
            e.currentTarget.style.background = isChildActive
              ? "#f8fafc"
              : "transparent";
        }}
        style={{
      ...
      >
        ...
        <span style={{ marginRight: 6, flexShrink: 0, display: "inline-flex" }}>
          <Icon
            as={node.icon}
            size={14}
            ...
            <div key={child.id} style={{ position: "relative" }}>
              ...
              <div
                ...
                style={{
                  display: "flex",
                  alignItems: "center",
                  padding: `5px 8px 5px ${8 + (depth + 1) * 18}px`,
                  cursor: "pointer",
                  borderRadius: 4,
                  margin: "1px 4px",
              ...
              >
                ...
                >
                  <Icon
                    as={child.icon}
                    size={14}
                    color={activeId === child.id ? "#2563eb" : "#64748b"}
                  />
                </span>
                <span
                  style={{
                    fontSize: 12,
                    flex: 1,
                    color: activeId === child.id ? "#2563eb" : "#374151",
...
const MENU_STRUCTURE = [
  {
    ...
    items: [
      ...
      {
        label: "Comprobar Presupuesto",
        action: "ComprobarPresupuesto",
        icon: ClipboardCheck,
      },
      {
        label: "Leer Presupuestos",
        action: "LeerPresupuestos",
        icon: FolderOpen,
      },
      ...
      {
        label: "Formato Simple Quote",
        action: "FormatoSimpleQuote",
        icon: Printer,
      },
      {
        label: "Crear SimpleQuote",
        action: "CrearSimpleQuote",
      ...
      { label: "Resumen", action: "Resumen", icon: BarChart3 },
      {
        ...
        action: "AplicarEstructura",
        icon: Palette,
      },
      {
        label: "Borrar Presupuesto actual",
        action: "BorrarPresupuestoActual",
        icon: X,
      },
      {
        label: "Comparar Presupuestos",
        action: "CompararPresupuestos",
        icon: Scale,
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        label: "Seleccionar Celdas",
        action: "SeleccionarCeldas",
        icon: Square,
      },
      {
        label: "Convertir selección en comentario",
        action: "ConvertirEnComentario",
        icon: MessageSquare,
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: "Buscar datos por Referencia",
        action: "BuscarDatosProductos",
        icon: Search,
      },
      {
        label: "Juntar productos duplicados",
        action: "JuntarDuplicados",
        icon: Layers,
      },
      { label: "---" },
      {
        label: "Quitar caracteres Referencia",
        action: "QuitarEspacios",
        icon: Scissors,
      },
      {
      ...
      {
        label: "Leer Precios de PMD",
        action: "LeerPreciosPMD",
        icon: Download,
      },
      {
        label: "Buscar Referencia SIEMENS",
        action: "BuscarReferencia",
        icon: Search,
      },
      {
        label: "Buscar equivalencia Competencia",
        action: "BuscarEquivalencia",
        icon: Repeat,
      },
      {
        label: "Calcular PVP a partir de GA",
        action: "CalcularPVP",
        icon: Calculator,
      },
    ...
    items: [
      {
        ...
        action: "AplicarDescuentos",
        icon: DollarSign,
      },
      {
        label: "Calcular Descuento",
        action: "CalcularDescuento",
        icon: Calculator,
      },
      {
        label: "Fijar el precio total",
        action: "FijarPrecioTotal",
        icon: Target,
      },
      {
        label: "Fijar precio de celdas",
        action: "FijarPrecioCeldas",
        icon: Hash,
      },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "---" },
      {
        ...
        action: "GestionarDescuentos",
        icon: Percent,
      },
      {
        label: "Gestionar BD Competencia",
        action: "GestionarProductosCompetencia",
        icon: Database,
      },
    ],
  },
...
const REPRESENTACION_OPCIONES = [
  {
    ...
    representacion: "TP",
    descripcion:
      "Total Posición - En la oferta aparecera un total con 1 unidad",
  },
  {
    id: 2,
    representacion: "Opcional",
    descripcion: "Posición Opcional - No sumada en el total del presupuesto.",
  },
  {
    id: 3,
    representacion: "Conf",
    descripcion: "Confirmar Posición. Sale resaltada la linea en la oferta.",
  },
];
...
const COLUMNS = [
  {
    key: "representacion",
    label: "Repres.",
    tooltip: "Representación",
    width: 50,
    type: "select",
    opciones: "representacion",
    align: "center",
  },
  {
    key: "naturaleza",
  ...
  { key: "nombre", label: "Producto", tooltip: "Producto", width: 320 },
  {
    key: "pvp",
    label: ["PVP", "Unitario"],
    tooltip: "PVP Unitario",
    width: 80,
    type: "number",
  },
  { key: "dtoaplicado", label: "Dto. %", width: 60, type: "number" },
  {
    key: "precionetounitario",
    label: ["Neto", "Unitario"],
    tooltip: "Neto Unitario",
    width: 80,
    type: "calc",
  },
  {
    key: "precionetoposicion",
    label: ["Neto", "Posición"],
    tooltip: "Neto Posición",
    width: 90,
    type: "calc",
  },
  { key: "descripcion", label: "Descripción", width: 260, wrap: true },
  {
    ...
    tooltip: "Grupo Descuento",
    width: 75,
  },
  { key: "familia", label: "Familia", width: 90 },
  { key: "subfamilia", label: "SubFamilia", width: 90 },
  {
    key: "preciocosteunitario",
    label: ["Coste Unit.", "(GA)"],
    tooltip: "Coste Unitario (GA)",
    width: 80,
    type: "number",
  },
  {
    key: "costeposicion",
    label: ["Coste", "Posición"],
    tooltip: "Coste Posición",
    width: 80,
    type: "calc",
  },
  ...
  {
    key: "precionetounitario2",
    label: ["Neto Unit.", "2"],
    tooltip: "Neto Unitario 2",
    width: 80,
    type: "number",
  },
];
  ...
  try {
    const guardado =
      typeof localStorage !== "undefined"
        ? localStorage.getItem("estilosNaturaleza")
        : null;
    if (guardado) {
  ...
  try {
    if (typeof localStorage !== "undefined") {
      localStorage.setItem(
        "presupuestoActual",
        JSON.stringify({ presupuesto, rows, fecha: new Date().toISOString() })
      );
      return true;
    }
  } catch (e) {
...
function compararPresupuestos(actual, rowsActual, bd) {
  ...
  if (filasActuales.length !== detalleBd.length) {
    diffs.push(
      `Número de filas: BD tiene ${detalleBd.length}, actual tiene ${filasActuales.length}`
    );
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!a) {
      diffs.push(
        `Fila ${filaActualNum}: solo en BD (${b.naturaleza || ""} ${
          b.referencia || b.nombre || ""
        })`
      );
      continue;
    }
    if (!b) {
      diffs.push(
        `Fila ${filaActualNum}: solo en actual (${a.naturaleza || ""} ${
...
function calcAnchosAuto(rows, apartados) {
  ...
  const minPorKey = {
    ...
    pvp: 75,
    dtoaplicado: 60,
    precionetounitario: 85,
    precionetoposicion: 95,
  };
  ...
  const labelLen = {
    ...
    pvp: 12,
    dtoaplicado: 6,
    precionetounitario: 12,
    precionetoposicion: 12,
  };
  const anchos = {};
  cols.forEach((key) => {
...
function calcNetoUnit(row) {
  return (row.pvp || 0) * (1 - (row.dtoaplicado || 0) / 100);
}
function calcNetoPos(row) {
  return calcNetoUnit(row) * (row.cantidad || 0);
}
function calcCostePos(row) {
  return (row.preciocosteunitario || 0) * (row.cantidad || 0);
}
function calcMargen(row) {
  const n = calcNetoPos(row),
    c = calcCostePos(row);
  return n ? ((n - c) / n) * 100 : 0;
}
function fmt(n) {
...
function calcApartados(rows) {
  ...
  rows.forEach((row) => {
    ...
    if (nat === "T1") {
      ...
      if (cnt[3] > 0) {
        elem[3]++;
        result[row.id] = join(cnt[0], cnt[1], cnt[2], cnt[3], elem[3]);
      } else if (cnt[2] > 0) {
        elem[2]++;
        result[row.id] = join(cnt[0], cnt[1], cnt[2], elem[2]);
      } else if (cnt[1] > 0) {
        elem[1]++;
        result[row.id] = join(cnt[0], cnt[1], elem[1]);
      } else if (cnt[0] > 0) {
        elem[0]++;
        result[row.id] = join(cnt[0], elem[0]);
      } else {
        elem[0]++;
        result[row.id] = join(1, elem[0]);
      }
    } else {
...
const AYUDA_TREE = [
  ...
  {
    id: "columnas",
    label: "Columnas de la grid",
    icon: BarChart3,
    children: [],
  },
  {
    id: "acciones",
    label: "Acciones de la grid",
    icon: MousePointer,
    children: [],
  },
...
function exportToExcel(
  ...
  const calcNetoUnit = (row) =>
    (row.pvp || 0) * (1 - (row.dtoaplicado || 0) / 100);
  ...
  const nombreFichero =
    `Oferta SIEMENS ${presupuesto.np} - ${presupuesto.titulo} - ${presupuesto.cliente}`.replace(
      /[\\/:*?"<>|]/g,
      "-"
    );
  ...
  aoa.push([
    null,
    `OFERTA SIEMENS — ${presupuesto.np} Rev.${presupuesto.revision}`,
  ]);
  aoa.push([]);
  ...
  aoa.push([
    ...
    null,
    null,
    `${presupuesto.np} Revisión ${presupuesto.revision}`,
  ]);
  aoa.push([]);
  ...
  rows.forEach((row) => {
    const nat = row.naturaleza;
    const ap =
      estructuraActiva && apartados[row.id]
        ? apartados[row.id]
        : row.posicion || "";
    ...
    if (esTitulo) {
      ...
      if (nat === "TT") {
        ...
        for (let i = idx - 1; i >= 0; i--) {
          ...
          if (["PD", "PE", "E"].includes(rows[i].naturaleza))
            suma += calcNetoPos(rows[i]);
        }
      }
      aoa.push([
        null,
      ...
      aoa.push([
        ...
        row.referencia || "",
        row.nombre || "",
        fmtNum(calcNetoUnit(row)),
        fmtNum(calcNetoPos(row)),
        row.descripcion || "",
      ]);
    } else {
      aoa.push([
...
const COLUMNAS_VALIDAS = [
  {
    key: "representacion",
    aliases: ["representacion", "repres", "rep", "representación"],
  },
  ...
  {
    key: "precionetounitario",
    aliases: ["neto unitario", "netounitario", "netounit"],
  },
  {
    key: "precionetoposicion",
    aliases: ["neto posicion", "netoposicion", "neto pos"],
  },
  {
    key: "descripcion",
    aliases: ["descripcion", "desc", "descr", "descripción"],
  },
  ...
  {
    key: "costeposicion",
    aliases: ["coste posicion", "costeposicion", "coste pos"],
  },
  ...
  {
    key: "precionetounitario2",
    aliases: ["neto unitario 2", "netounitario2", "neto2", "neto unit 2"],
  },
  {
    key: "grupodescuento",
    aliases: ["grupo descuento", "grupodescuento", "grupodto", "gdto"],
  },
];
  ...
  headers.forEach((header, idx) => {
    ...
    const candidatos = COLUMNAS_VALIDAS.filter((col) =>
      col.aliases.some((alias) => {
        ...
        return (
          a === normalizado ||
          a.startsWith(normalizado) ||
          normalizado.startsWith(a)
        );
      })
    );
    ...
    if (exacta.length === 1) {
      ...
      errores.push(
        `Columna "${header}" coincide exactamente con varias columnas válidas`
      );
    } else if (candidatos.length === 1) {
...
function ImportarDialog({ onClose, onImport }) {
  ...
  const handleFile = async (file) => {
    if (!file) return;
    addLog(
      `Cargando fichero "${file.name}" (${(file.size / 1024).toFixed(
        1
      )} KB)...`,
      "info"
    );
  ...
  const handlePasted = () => {
    ...
    addLog(
      `Procesando texto pegado (${pastedText.length} caracteres)...`,
      "info"
    );
  ...
  const procesar = (aoa, origen = "fichero") => {
    ...
    const dataRows = aoa
      ...
      .filter((r) => r.some((c) => String(c || "").trim() !== ""));
    addLog(
      `Detectadas ${headers.length} columnas y ${dataRows.length} filas de datos`,
      "info"
    );
    ...
    addLog(
      `Columnas reconocidas: ${reconocidas.join(", ") || "ninguna"}`,
      reconocidas.length > 0 ? "success" : "warning"
    );
    if (errores.length > 0) {
      errores.forEach((e) => addLog("AVISO: " + e, "warning"));
    } else {
      addLog(
        "Validación correcta. Pulsa 'Confirmar e insertar' para añadir las filas al presupuesto.",
        "success"
      );
    }
  ...
  const confirmar = () => {
    ...
    addLog(
      `Insertando ${previewData.rows.length} filas en el presupuesto...`,
      "info"
    );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon as={Download} size={18} color="#1e3a5f" /> Importar
        ...
        {step === "input" && (
          <>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                1. Arrastra o selecciona un fichero Excel
              </label>
              <div
                onDragOver={(e) => {
                  e.preventDefault();
              ...
              >
                ...
                <div style={{ marginTop: 6 }}>
                  {dragOver ? (
                    ...
                    <>
                      Arrastra un Excel/CSV (.xlsx, .xls, .csv), o haz click
                      para elegir uno.
                    </>
                  )}
                </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                2. Pegar contenido (copiar/pegar desde Excel)
              </label>
              <textarea
                value={pastedText}
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: "none",
                  background: pastedText.trim() ? "#2563eb" : "#cbd5e1",
                  color: "#fff",
            ...
            >
              <strong>Formato esperado:</strong> primera fila con nombres de
              columna (referencia, cantidad, descripcion...). Se permiten
              abreviaturas (Ref, Can, Desc) pero no ambiguas: "re" sería ambigua
              entre <em>referencia</em> y <em>representación</em>.
            </div>
          </>
        )}
        ...
        {step === "preview" && previewData && (
          <>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                Vista previa ({previewData.rows.length} filas):
              </div>
              <div
                style={{
                  maxHeight: 200,
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: "none",
                  background: errores.length === 0 ? "#16a34a" : "#cbd5e1",
                  color: "#fff",
                  cursor: errores.length === 0 ? "pointer" : "default",
                  fontSize: 12,
                  fontWeight: 600,
                }}
              >
                <BtnContent icon={Check} iconColor="#fff">
                  Importar {previewData.rows.length} filas
                </BtnContent>
              </button>
            </div>
          </>
...
function exportarDatosExcel(presupuesto, rows) {
  const calcNetoUnit = (row) =>
    (row.pvp || 0) * (1 - (row.dtoaplicado || 0) / 100);
  const calcNetoPos = (row) => calcNetoUnit(row) * (row.cantidad || 0);
  const calcCostePos = (row) =>
    (row.preciocosteunitario || 0) * (row.cantidad || 0);
  const calcMargen = (row) => {
    const n = calcNetoPos(row),
      c = calcCostePos(row);
    return n ? ((n - c) / n) * 100 : 0;
  };
  ...
  const nombreFichero =
    `Datos Presupuesto ${presupuesto.np} - ${presupuesto.titulo} - ${presupuesto.cliente}`.replace(
      /[\\/:*?"<>|]/g,
      "-"
    );
  ...
  const totalPVPpos = productos.reduce(
    (s, r) => s + (r.pvp || 0) * (r.cantidad || 0),
    0
  );
  const totalNeto = productos.reduce((s, r) => s + calcNetoPos(r), 0);
  const totalCoste = productos.reduce((s, r) => s + calcCostePos(r), 0);
  const margenTotal = totalNeto
    ? ((totalNeto - totalCoste) / totalNeto) * 100
    : 0;
  const dtoMedio = totalPVPpos
    ? ((totalPVPpos - totalNeto) / totalPVPpos) * 100
    : 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Selecciona una columna. Se borrarán todas las filas seleccionadas (con
          el checkbox marcado) cuyo valor en esa columna esté vacío.
        </p>
        ...
        >
          <strong>{lineasSeleccionadas.length}</strong> fila
          {lineasSeleccionadas.length !== 1 ? "s" : ""} marcada
          {lineasSeleccionadas.length !== 1 ? "s" : ""} con el checkbox
        </div>
        ...
        >
          ...
          <button
            onClick={aceptar}
            disabled={!columna || lineasSeleccionadas.length === 0}
            style={{
              padding: "7px 20px",
              borderRadius: 6,
              border: "none",
              background:
                columna && lineasSeleccionadas.length > 0
                  ? "#dc2626"
                  : "#cbd5e1",
              color: "#fff",
              cursor:
          ...
          >
            <BtnContent icon={Trash2} iconColor="#fff">
              Borrar
            </BtnContent>
          </button>
        </div>
        ...
        {confirmar && (
          ...
          >
            ...
            >
              ...
              >
                Se borrarán <strong>{cantidadBorrar}</strong> fila
                {cantidadBorrar !== 1 ? "s" : ""} que tienen la columna{" "}
                <strong>{labelColumna}</strong> vacía.
                {cantidadBorrar === 0 && (
                  <>
                    <br />
                    <span style={{ color: "#94a3b8" }}>
                      No hay filas con esa columna vacía entre las
                      seleccionadas.
                    </span>
                  </>
                )}
              ...
              >
                ...
                <button
                  ...
                  disabled={cantidadBorrar === 0}
                  style={{
                    padding: "7px 16px",
                    borderRadius: 6,
                    border: "none",
                    background: cantidadBorrar > 0 ? "#dc2626" : "#cbd5e1",
                    color: "#fff",
                    cursor: cantidadBorrar > 0 ? "pointer" : "default",
                    fontSize: 13,
                    fontWeight: 600,
                  }}
                >
...
function FijarPrecioDialog({
  ...
  const aplicar = () => {
    ...
    if (totales.netoTotal === 0) {
      setError(
        "El neto actual de las líneas seleccionadas es 0, no se puede repartir"
      );
      return;
    }
    ...
    if (alguno_fuera_rango) {
      setError(
        "El reparto generaría descuentos fuera del rango -100% a 100% en alguna línea"
      );
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <div>
              <span style={{ color: "#64748b" }}>Líneas afectadas:</span>{" "}
              <strong>{lineasAfectadas.length}</strong>
            </div>
            <div>
              <span style={{ color: "#64748b" }}>PVP total:</span>{" "}
              <strong>{fmt(totales.pvpTotal)} €</strong>
            </div>
            <div>
              <span style={{ color: "#64748b" }}>Neto actual:</span>{" "}
              <strong style={{ color: "#0369a1" }}>
                {fmt(totales.netoTotal)} €
              </strong>
            </div>
            <div>
              <span style={{ color: "#64748b" }}>Dto. medio actual:</span>{" "}
              <strong>
                {totales.pvpTotal > 0
                  ? fmt(
                      ((totales.pvpTotal - totales.netoTotal) /
                        totales.pvpTotal) *
                        100
                    )
                  : "0,00"}
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          />
          <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
            Se ajustará el descuento de cada línea para que su neto se reparta
            proporcionalmente respecto al neto actual y el total sea exactamente
            este importe.
          </div>
        </div>
        ...
        >
          ...
          <button
            onClick={aplicar}
            disabled={!importe || lineasAfectadas.length === 0}
            style={{
              padding: "7px 20px",
              borderRadius: 6,
              border: "none",
              background:
                importe && lineasAfectadas.length > 0 ? "#16a34a" : "#cbd5e1",
              color: "#fff",
              cursor:
                importe && lineasAfectadas.length > 0 ? "pointer" : "default",
              fontSize: 12,
              fontWeight: 600,
            }}
          >
...
function AplicarDescuentosDialog({ rows, onClose, onApply }) {
  ...
  const dglFor = (codigo) => {
    ...
    const g = gruposBD.find(
      (x) =>
        String(x.grupodescuentospain || "")
          .trim()
          .toUpperCase() === cod
    );
  ...
  const grupos = (() => {
    ...
    return Object.values(mapa).sort((a, b) => b.importeNeto - a.importeNeto);
  })();
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon as={DollarSign} size={18} color="#1e3a5f" /> Aplicar
            descuentos por Grupo Descuento
          </h2>
          <button
            onClick={onClose}
            style={{
              ...
              >
                ...
                <tbody>
                  {grupos.map((g, i) => {
                    const isSel =
                      seleccionada &&
                      seleccionada.grupodescuento === g.grupodescuento &&
                      seleccionada.dtoActual === g.dtoActual;
                    return (
                      <tr
                        key={i}
                      ...
                      >
                        ...
                        {(() => {
                          ...
                          let dtoColor = "#475569";
                          let dtoWeight = 400;
                          if (d.dgl2 != null && g.dtoActual > Number(d.dgl2)) {
                            dtoColor = "#dc2626";
                            dtoWeight = 700;
                          } else if (
                            d.dgl1 != null &&
                            g.dtoActual > Number(d.dgl1)
                          ) {
                            dtoColor = "#ca8a04";
                            dtoWeight = 600;
                          }
        ...
        {seleccionada && (
          ...
          >
            <div style={{ fontSize: 12, color: "#1e3a5f", marginBottom: 8 }}>
              Grupo seleccionado: <strong>{seleccionada.grupodescuento}</strong>{" "}
              con dto. actual <strong>{fmt(seleccionada.dtoActual)}%</strong> (
              {seleccionada.lineas} línea{seleccionada.lineas !== 1 ? "s" : ""})
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <input
                ...
                step="1"
                value={nuevoDto}
                onChange={(e) => setNuevoDto(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") aplicar();
                }}
                autoFocus
        ...
        >
          ...
          <button
            onClick={aplicar}
            disabled={!seleccionada || nuevoDto === ""}
            style={{
              padding: "7px 20px",
              borderRadius: 6,
              border: "none",
              background:
                seleccionada && nuevoDto !== "" ? "#16a34a" : "#cbd5e1",
              color: "#fff",
              cursor: seleccionada && nuevoDto !== "" ? "pointer" : "default",
              fontSize: 12,
              fontWeight: 600,
            }}
          >
...
function LeerPresupuestosDialog({ onClose, onCargar, setStatus }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/presupuestos/${seleccionado.id}/completo`)
      ...
      .catch(
        (e) =>
          setStatus &&
          setStatus("Error cargando detalle: " + e.message, "error")
      )
  ...
  const cargarLista = async () => {
    ...
    setStatus &&
      setStatus(
        busqueda.trim()
          ? `Buscando "${busqueda.trim()}" en presupuestos...`
          : "Cargando lista de presupuestos...",
        "working"
      );
    try {
      const url = busqueda.trim()
        ? `${API_URL}/presupuestos/?busqueda=${encodeURIComponent(
            busqueda.trim()
          )}`
      ...
      setPresupuestos([]);
      setStatus &&
        setStatus("Error al cargar presupuestos: " + e.message, "error");
    } finally {
      setCargando(false);
    }
  };
  ...
  const borrarPresupuesto = async () => {
    ...
    setStatus &&
      setStatus(
        `Borrando presupuesto ${
          seleccionado.numerocompleto || seleccionado.numero
        }...`,
        "working"
      );
    try {
      const res = await fetch(`${API_URL}/presupuestos/${seleccionado.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
      ...
      setStatus &&
        setStatus("Error borrando presupuesto: " + e.message, "error");
    } finally {
      setBorrando(false);
    }
  };
  ...
  const leerSeleccionado = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/presupuestos/${seleccionado.id}/completo`
      );
      if (!res.ok) throw new Error("Error " + res.status);
      const data = await res.json();
      onCargar(data);
      ...
      setStatus &&
        setStatus("Error al leer presupuesto: " + e.message, "error");
    } finally {
      setCargando(false);
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon as={FolderOpen} size={18} color="#1e3a5f" /> Leer presupuesto
        ...
        >
          <button
            ...
            style={{
              ...
              borderBottom:
                tab === "lista" ? "2px solid #171717" : "2px solid transparent",
              marginBottom: "-1px",
            }}
          >
            <BtnContent
              icon={FolderOpen}
              iconColor={tab === "lista" ? "#171717" : "#737373"}
            >
                    ...
                    presupuestos.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {(Number(p.totalpresupuesto) || 0).toLocaleString(
                              "es-ES",
                              {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
        ...
        {tab === "detalle" && (
          ...
          >
            {!seleccionado ? (
              ...
              >
                No hay presupuesto seleccionado. Vuelve a la lista y selecciona
                uno.
              </div>
            ) : cargandoDetalle ? (
              <div
              ...
              >
                <Icon as={RefreshCw} size={18} color="#64748b" /> Cargando
                detalle...
              </div>
            ) : detalle ? (
              <>
                ...
                >
                  <div>
                    ...
                    <span style={{ color: "#171717", fontWeight: 600 }}>
                      {detalle.cabecera.numerocompleto ||
                        detalle.cabecera.numero}
                    </span>
                  </div>
                  <div>
                    <strong
                  ...
                  <div>
                    ...
                    <br />
                    <span style={{ color: "#171717" }}>
                      {detalle.cabecera.nombrecomun ||
                        detalle.cabecera.razonsocial ||
                        "—"}
                    </span>
                  </div>
                </div>
                    ...
                    <tbody>
                      ...
                      {(detalle.detalle || []).map((d, i) => (
                        ...
                        >
                          ...
                          <td style={{ padding: "4px 8px", color: "#171717" }}>
                            {d.nombre || ""}
                          </td>
                          <td
                            style={{
                          ...
                          >
                            {(Number(d.dtoaplicado) || 0).toLocaleString(
                              "es-ES",
                          ...
                          >
                            {(Number(d.precionetoposicion) || 0).toLocaleString(
                              "es-ES",
                              {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                          ...
                          <td style={{ padding: "4px 8px", color: "#737373" }}>
                            {d.familia || ""}
                          </td>
                          <td style={{ padding: "4px 8px", color: "#737373" }}>
                            {d.subfamilia || ""}
                ...
                >
                  {(detalle.detalle || []).length} línea
                  {(detalle.detalle || []).length !== 1 ? "s" : ""}
                </div>
              </>
            ) : null}
          </div>
        ...
        >
          <button
            ...
            style={{
              padding: "7px 14px",
              borderRadius: 6,
              border: seleccionado ? "1px solid #fca5a5" : "1px solid #d4d4d4",
              background: seleccionado ? "#fef2f2" : "#f5f5f5",
              color: seleccionado ? "#991b1b" : "#737373",
              cursor: seleccionado ? "pointer" : "default",
              fontSize: 12,
            }}
          >
            <BtnContent
              icon={Trash2}
              iconColor={seleccionado ? "#dc2626" : "#737373"}
            >
              Borrar Presupuesto
            </BtnContent>
          </button>
              ...
              style={{
                ...
                border:
                  seleccionado && !cargando
                    ? "1px solid #16a34a"
                    : "1px solid #d4d4d4",
                background: seleccionado && !cargando ? "#dcfce7" : "#f5f5f5",
                color: seleccionado && !cargando ? "#14532d" : "#737373",
                cursor: seleccionado && !cargando ? "pointer" : "default",
                fontSize: 12,
                fontWeight: 600,
              }}
            >
              <BtnContent
                icon={Check}
                iconColor={seleccionado && !cargando ? "#14532d" : "#737373"}
              >
                Leer Presupuesto
              </BtnContent>
            </button>
        ...
        {confirmBorrar && seleccionado && (
          ...
          >
            ...
            >
              ...
              >
                Vas a borrar el presupuesto{" "}
                <strong style={{ color: "#171717" }}>
                  {seleccionado.numerocompleto || seleccionado.numero}
                </strong>{" "}
                Rev.{seleccionado.revision ?? 0} de la base de datos.
              </p>
              <div
                style={{
                  background: "#f8fafc",
              ...
              >
                <div>
                  <strong>Título:</strong> {seleccionado.titulo}
                </div>
                <div>
                  <strong>Cliente:</strong>{" "}
                  {seleccionado.nombrecomun || seleccionado.razonsocial}
                </div>
              </div>
              <div
                style={{
              ...
              >
                <strong>Atención:</strong> esta acción borrará el presupuesto
                completo y todas sus líneas de detalle. No se puede deshacer.
              </div>
              <div
                style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}
              >
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  Confirmar lectura del presupuesto
                </h3>
              </div>
              <p
                style={{
              ...
              >
                Vas a leer el presupuesto{" "}
                <strong>
                  {seleccionado.numerocompleto || seleccionado.numero}
                </strong>{" "}
                Rev.{seleccionado.revision ?? 0}:
              </p>
              <div
                style={{
                  background: "#f8fafc",
              ...
              >
                ...
                <strong>
                  sobreescribirá todos los datos del presupuesto en pantalla
                </strong>
                .
              </div>
              <div
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    border: "1px solid #16a34a",
                    background: cargando ? "#f1f5f9" : "#dcfce7",
                    color: "#14532d",
                ...
                >
                  <BtnContent
                    icon={cargando ? RefreshCw : Check}
                    iconColor="#14532d"
                  >
                    {cargando ? "Cargando..." : "Sí, leer y sobreescribir"}
                  </BtnContent>
                </button>
              </div>
            </div>
...
function ContactosDialog({ onClose, setStatus }) {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setStatus &&
        setStatus(
          `${data.length} contacto${data.length !== 1 ? "s" : ""} cargado${
            data.length !== 1 ? "s" : ""
          }`,
          "success"
        );
  ...
  const guardar = async () => {
    ...
    try {
      const url =
        seleccionado === "nuevo"
          ? `${API_URL}/contactos/`
          : `${API_URL}/contactos/${seleccionado}`;
      const method = seleccionado === "nuevo" ? "POST" : "PUT";
      const res = await fetch(url, {
        method,
        headers: { "Content-Type": "application/json" },
  ...
  const nombreCliente = (id) => {
    ...
    return cl
      ? cl.nombrecomun || cl.razonsocial || `Cliente ${id}`
      : `Cliente ${id}`;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <label style={{ fontSize: 12, color: "#525252", fontWeight: 500 }}>
            Cliente:
          </label>
          <select
          ...
          >
            ...
            {clientes.map((c) => (
              <option key={c.id} value={c.id}>
                {c.nombrecomun || c.razonsocial || `Cliente ${c.id}`}
              </option>
            ))}
          </select>
          <input
                    ...
                    {filtrados.map((c) => (
                      <tr
                        ...
                        style={{
                          borderBottom: "1px solid #f1f5f9",
                          background:
                            seleccionado === c.id ? "#eff6ff" : "transparent",
                        }}
                      >
                        <td
                          style={{
                        ...
                        <td style={{ padding: "6px 8px", color: "#525252" }}>
                          {c.cargo || "—"}
                        </td>
                        <td
                          style={{
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              setConfirmBorrar({ id: c.id, nombre: c.nombre })
                            }
                            title="Borrar"
                            style={{
                              padding: "3px 7px",
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: 13, color: "#525252", marginBottom: 16 }}>
                ...
                <strong>{confirmBorrar.nombre}</strong>?
              </p>
              <div
                style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}
              >
                ...
                <button
                  onClick={() => borrar(confirmBorrar.id, confirmBorrar.nombre)}
                  style={{
                    padding: "6px 14px",
                    borderRadius: 6,
                    border: "1px solid #dc2626",
  ...
  return (
    ...
    >
      ...
      >
              ...
              <tbody>
                {filtrados.map((c) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "5px 8px", color: "#525252" }}>
                      {c.poblacion || "—"}
                    </td>
                  </tr>
                ))}
        ...
        >
          {filtrados.length} cliente{filtrados.length !== 1 ? "s" : ""} · doble
          clic o un clic para seleccionar
        </div>
      </div>
    </div>
  );
...
function ClientesDialog({ onClose, setStatus, onAsignarPresupuesto }) {
  ...
  const cargar = async () => {
    ...
    setError(null);
    setStatus &&
      setStatus("Cargando clientes desde la base de datos...", "working");
    try {
      ...
      setStatus &&
        setStatus(
          `${data.length} cliente${data.length !== 1 ? "s" : ""} cargado${
            data.length !== 1 ? "s" : ""
          }`,
          "success"
        );
  ...
  const cambios = (() => {
    ...
    clientes.forEach((c) => {
      ...
      if (!orig) return;
      const cambiado = Object.keys(c).some(
        (k) => String(c[k] ?? "") !== String(orig[k] ?? "")
      );
  ...
  const guardarCelda = () => {
    ...
    setClientes((cs) =>
      cs.map((c) =>
        c.id === editingCell.id ? { ...c, [editingCell.key]: valor } : c
      )
    );
    setEditingCell(null);
  };
  ...
  const copiarCliente = () => {
    ...
    const copia = {
      ...
      razonsocial: (src.razonsocial || "") + " (copia)",
    };
  ...
  const guardarTodo = async () => {
    ...
    setStatus &&
      setStatus(
        `Guardando ${cambios.length} cambio${
          cambios.length > 1 ? "s" : ""
        } en la base de datos...`,
        "working"
      );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon as={Users} size={18} color="#1e3a5f" /> Gestión de clientes
          </h2>
          <button
            onClick={onClose}
            style={{
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border: "1px solid #cbd5e1",
              background: seleccionado ? "#fff" : "#fafafa",
              color: seleccionado ? "#1e293b" : "#cbd5e1",
              cursor: seleccionado ? "pointer" : "default",
            }}
          >
          ...
          <button
            onClick={() => {
              ...
              setStatus &&
                setStatus(
                  `Cliente "${
                    cliente.nombrecomun || cliente.razonsocial
          ...
          >
            <Icon
              as={Check}
              size={16}
              color={seleccionado ? "#16a34a" : "#cbd5e1"}
            />
            <span>
              Asignar Cliente a<br />
              Presupuesto actual
            </span>
          </button>
          {cambios.length > 0 && (
            ...
            >
              ● {cambios.length} cambio{cambios.length > 1 ? "s" : ""} sin
              guardar
            </span>
          )}
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {CLIENTE_COLS.map((col) => {
                        ...
                        const display =
                          isNuevo && col.key === "id" ? "NUEVO" : valor ?? "";
                        return (
                          <td
                            key={col.key}
                            onDoubleClick={() =>
                              !col.readonly && iniciarEdicion(c.id, col.key)
                            }
                            style={{
                              padding: 0,
                              width: col.width,
                              ...
                              background: col.readonly ? "#f8fafc" : "inherit",
                            }}
                          >
                            {isEditing ? (
                              <input
        ...
        >
          ...
          <span style={{ marginLeft: "auto" }}>
            Doble click en una celda para editar. {filtrados.length} cliente
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              background:
                cambios.length > 0 && !guardando ? "#16a34a" : "#cbd5e1",
              color: "#fff",
              cursor: cambios.length > 0 && !guardando ? "pointer" : "default",
              fontSize: 12,
              fontWeight: 600,
            }}
          >
            <BtnContent icon={Save} iconColor="#fff">
              Guardar cambios{cambios.length > 0 ? ` (${cambios.length})` : ""}
            </BtnContent>
          </button>
        </div>
        ...
        {confirmGuardar && (
          ...
          >
            ...
            >
              ...
              >
                Se aplicarán <strong>{cambios.length}</strong> cambio
                {cambios.length > 1 ? "s" : ""} en la base de datos:
              </p>
              <ul
                style={{
                  fontSize: 12,
              ...
              >
                {cambios.map((c) => (
                  <li key={c.id}>
                    {c.id < 0 ? (
                      <strong style={{ color: "#16a34a" }}>NUEVO: </strong>
                    ) : (
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 16px" }}>
          Indica el rango de filas a marcar. Se seleccionarán los checkbox de
          todas las filas dentro del rango.
        </p>
        ...
        >
          ...
          >
            <label style={{ fontSize: 11, fontWeight: 600, color: "#1e3a5f" }}>
              Desde la fila
            </label>
            <input
              type="number"
          ...
          >
            ...
            <input
              ...
              max={totalFilas}
              value={hastaFila}
              onChange={(e) => setHastaFila(e.target.value)}
              onKeyDown={(e) => {
        ...
        >
          ...
          <strong>
            {totalFilas} fila{totalFilas !== 1 ? "s" : ""}
          </strong>
          .
        </div>
...
function AutocompleteReferencia({
  value,
  onChange,
  onConfirm,
  onCancel,
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const ordenados = [...data].sort((a, b) => {
          const aStarts = String(a.referencia || "")
            .toUpperCase()
            .startsWith(txtUp)
            ? 0
            : 1;
          const bStarts = String(b.referencia || "")
            .toUpperCase()
            .startsWith(txtUp)
            ? 0
            : 1;
          if (aStarts !== bStarts) return aStarts - bStarts;
          return String(a.referencia || "").localeCompare(
            String(b.referencia || "")
          );
        });
    ...
    <div style={{ position: "relative", width: "100%" }}>
        ...
        onKeyDown={(e) => {
          if (e.key === "ArrowDown" && open) {
            ...
            if (open && suggestions[highlighted])
              elegir(suggestions[highlighted]);
      ...
      {open && suggestions.length > 0 && (
        ...
        >
          {loading && (
            <div
              style={{ padding: "6px 10px", fontSize: 11, color: "#737373" }}
            >
              Buscando...
            </div>
          )}
          {suggestions.map((s, i) => (
            ...
            >
              ...
              >
                {(Number(s.pvp) || 0).toLocaleString("es-ES", {
                  minimumFractionDigits: 2,
                })}{" "}
                €
              </span>
...
function LeerProductoDialog({ onClose, onInsertar, setStatus }) {
  ...
  const cargarLista = async () => {
    ...
    try {
      const LIMITE = 200;
      const params = busqueda.trim()
        ? `busqueda=${encodeURIComponent(
            busqueda.trim()
          )}&campo=${campo}&limite=${LIMITE}`
        : `limite=${LIMITE}`;
  ...
  const copiarReferencia = async () => {
    ...
    try {
      ...
      setStatus &&
        setStatus(
          `Referencia ${seleccionado.referencia} copiada al portapapeles`,
          "success"
        );
  ...
  const insertar = () => {
    ...
    setStatus &&
      setStatus(`Producto ${seleccionado.referencia} insertado`, "success");
    onClose();
  };
  ...
  const borrarProducto = async () => {
    ...
    setBorrando(true);
    setStatus &&
      setStatus(`Borrando producto "${seleccionado.referencia}"...`, "working");
    try {
      const res = await fetch(`${API_URL}/productos/${seleccionado.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
      ...
      setStatus &&
        setStatus(
          `Producto "${seleccionado.referencia}" borrado de la base de datos`,
          "success"
        );
      setConfirmBorrar(false);
      setSeleccionado(null);
      ...
      >
          ...
          <input
            ...
            placeholder={
              campo === "referencia"
                ...
                : "Buscar en referencia, nombre y descripción..."
            }
            autoFocus
            style={{
              flex: 1,
        ...
        <div
          style={{
            ...
            border: "1px solid " + (seleccionado ? "#bae6fd" : "#e2e8f0"),
            borderRadius: 6,
            flexShrink: 0,
          }}
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#0369a1" }}>
              Referencia
            </label>
            <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
              ...
              <button
                ...
                style={{
                  padding: "5px 8px",
                  border: "1px solid " + (copiado ? "#16a34a" : "#cbd5e1"),
                  borderRadius: 4,
                  background: copiado ? "#dcfce7" : "#fff",
                  cursor: seleccionado ? "pointer" : "default",
                  color: seleccionado ? "#1e3a5f" : "#cbd5e1",
                }}
              >
                <Icon
                  ...
                  size={14}
                  color={
                    copiado ? "#16a34a" : seleccionado ? "#475569" : "#cbd5e1"
                  }
                />
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            >
              <tr>
                <th
                  style={{ width: 32, padding: "8px 6px", color: "#171717" }}
                ></th>
                <th
                  style={{
                    padding: "8px 10px",
                ...
                productos.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {(Number(p.pvp) || 0).toLocaleString("es-ES", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}{" "}
                        €
        ...
        >
          <button
            onClick={async () => {
              ...
              setComprobandoUso(true);
              setStatus &&
                setStatus(`Comprobando referencias del producto...`, "working");
              try {
                const res = await fetch(
                  `${API_URL}/productos/${seleccionado.id}/uso`
                );
                if (!res.ok) throw new Error("HTTP " + res.status);
                const data = await res.json();
                if (!data.borrable) {
                  setStatus &&
                    setStatus(
                      `No se puede borrar "${seleccionado.referencia}": está referenciado en ${data.en_presupuestos} línea(s) de presupuesto y ${data.en_elementos} elemento(s).`,
                      "error"
                    );
                  return;
                }
                setStatus &&
                  setStatus(
                    "Producto sin referencias, listo para borrar",
                    "info"
                  );
                setConfirmBorrar(true);
              } catch (e) {
                setStatus &&
                  setStatus("Error comprobando uso: " + e.message, "error");
              } finally {
                setComprobandoUso(false);
              }
            }}
            ...
            style={{
              ...
              borderRadius: 6,
              border:
                seleccionado && !comprobandoUso
                  ? "1px solid #fca5a5"
                  : "1px solid #d4d4d4",
              background:
                seleccionado && !comprobandoUso ? "#fef2f2" : "#f5f5f5",
              color: seleccionado && !comprobandoUso ? "#991b1b" : "#737373",
              cursor: seleccionado && !comprobandoUso ? "pointer" : "default",
              fontSize: 12,
            }}
          >
            <BtnContent
              icon={comprobandoUso ? RefreshCw : Trash2}
              iconColor={
                seleccionado && !comprobandoUso ? "#dc2626" : "#737373"
              }
            >
              {comprobandoUso ? "Comprobando..." : "Borrar Producto"}
            </BtnContent>
          </button>
          <div style={{ display: "flex", gap: 8 }}>
            <button
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: "none",
                background: seleccionado ? "#16a34a" : "#cbd5e1",
                color: "#fff",
                cursor: seleccionado ? "pointer" : "default",
                fontSize: 12,
                fontWeight: 600,
        ...
        {confirmBorrar && seleccionado && (
          ...
          >
            ...
            >
              ...
              >
                Vas a borrar el producto{" "}
                <strong style={{ color: "#171717", fontFamily: "monospace" }}>
              ...
              >
                ...
                {seleccionado.descripcion && (
                  <div>
                    ...
                    {String(seleccionado.descripcion).substring(0, 100)}
                    {seleccionado.descripcion.length > 100 ? "..." : ""}
                  </div>
                )}
              </div>
              <div
              ...
              >
                <strong>Atención:</strong> esta acción borrará el producto de la
...
function GuardarElementoDialog({ filasSeleccionadas, onClose, setStatus }) {
  ...
  const productos = filasSeleccionadas.filter(
    (f) => ["PD", "PE", "E"].includes(f.naturaleza) && f.referencia
  );
  ...
  const guardar = async () => {
    ...
    setStatus &&
      setStatus(
        `Guardando elemento "${nombre}" con ${productos.length} producto${
    ...
    try {
      ...
      setStatus &&
        setStatus(
          `Elemento "${nombre}" guardado con ${data.productos_insertados} productos`,
          "success"
        );
    } catch (e) {
      setError(e.message);
      ...
      >
        ...
        {!resultado ? (
          <>
            ...
            >
              <strong>{productos.length}</strong> línea
              {productos.length !== 1 ? "s" : ""} válida
              {productos.length !== 1 ? "s" : ""} (productos + comentarios) de
              las {filasSeleccionadas.length} fila
              {filasSeleccionadas.length !== 1 ? "s" : ""} seleccionada
              {filasSeleccionadas.length !== 1 ? "s" : ""}.
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              >
                {productos.length === 0 ? (
                  ...
                  >
                    No hay filas con referencia válida
                  </div>
                ) : (
                  productos.map((p, i) => (
                    <div
                    ...
                    >
                      ...
                      <strong style={{ color: "#1e3a5f", minWidth: 150 }}>
                        {p.referencia}
                      </strong>
                      <span
                        style={{
            ...
            >
              ...
              <button
                onClick={guardar}
                disabled={guardando || !nombre.trim() || productos.length === 0}
                style={{
                  padding: "7px 20px",
                  borderRadius: 6,
                  border: "none",
                  background:
                    !guardando && nombre.trim() && productos.length > 0
                      ? "#16a34a"
                      : "#cbd5e1",
                  color: "#fff",
                  cursor:
          ...
          <>
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#15803d" }}>
                ID: <strong>{resultado.id}</strong> — Nombre:{" "}
                <strong>{resultado.nombre}</strong>
                <br />
                Productos insertados:{" "}
                <strong>{resultado.productos_insertados}</strong>
              </div>
            </div>

            {resultado.referencias_no_encontradas &&
              resultado.referencias_no_encontradas.length > 0 && (
                <div
                  style={{
                    background: "#fef3c7",
                    border: "1px solid #fcd34d",
                ...
                >
                  <strong>⚠ Atención:</strong> las siguientes referencias no se
                  encontraron en la BD y no se guardaron:
                  <ul style={{ margin: "6px 0 0 16px", padding: 0 }}>
                    {resultado.referencias_no_encontradas.map((r, i) => (
                      <li key={i}>
                        <code>{r}</code>
                      </li>
                    ))}
...
function LeerElementoDialog({ onClose, onInsertar, setStatus }) {
    ...
    fetch(`${API_URL}/elementos/${seleccionado.id}`)
      ...
      .catch(
        (e) =>
          setStatus &&
          setStatus(
            "Error cargando detalle del elemento: " + e.message,
            "error"
          )
      )
      .finally(() => setCargandoDetalle(false));
  ...
  const cargarLista = async () => {
    ...
    try {
      const url = busqueda.trim()
        ? `${API_URL}/elementos/?busqueda=${encodeURIComponent(
            busqueda.trim()
          )}`
        : `${API_URL}/elementos/`;
      const res = await fetch(url);
  ...
  const toggleSel = (el, evt, idx) => {
    setSelIds((prev) => {
      ...
      if (evt.shiftKey && ultimoSelIdx !== null && idx !== null) {
        ...
        const ini = Math.min(ultimoSelIdx, idx),
          fin = Math.max(ultimoSelIdx, idx);
  ...
  const insertar = async () => {
    ...
    setInsertando(true);
    setStatus &&
      setStatus(`Cargando elemento "${seleccionado.nombre}"...`, "working");
    try {
      ...
      setStatus &&
        setStatus(
          `Elemento "${seleccionado.nombre}" insertado con ${data.productos.length} productos`,
          "success"
        );
      onClose();
    } catch (e) {
      setError(e.message);
      setStatus &&
        setStatus("Error insertando elemento: " + e.message, "error");
    } finally {
      setInsertando(false);
    }
  };
  ...
  const borrarElementos = async () => {
    ...
    setStatus &&
      setStatus(
        `Borrando ${ids.length} elemento${ids.length !== 1 ? "s" : ""}...`,
        "working"
      );
    let ok = 0,
      errs = 0;
  ...
  const aplicarGrupo = async () => {
    ...
    setStatus &&
      setStatus(
        `Cambiando grupo a ${ids.length} elemento${
          ids.length !== 1 ? "s" : ""
        }...`,
        "working"
      );
    ...
    setStatus &&
      setStatus(
        `Grupo cambiado: ${ok} OK, ${errs} con error`,
        errs > 0 ? "error" : "success"
      );
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      if (editar.descripcion !== undefined)
        payload.descripcion = editar.descripcion;
      const res = await fetch(`${API_URL}/elementos/${seleccionado.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontSize: 12,
              fontWeight: 600,
              cursor: cantSel === 1 ? "pointer" : "default",
              color:
                cantSel !== 1
                  ? "#cbd5e1"
                  : tab === "detalle"
          ...
          >
            ...
            >
              ...
              {cantSel === 1 && seleccionado ? `(${seleccionado.nombre})` : ""}
            </BtnContent>
          </button>
        </div>
                    ...
                    elementos.map((el, idx) => {
                      ...
                      return (
                        <tr
                          ...
                          onDoubleClick={() => {
                            setSelIds(new Set([el.id]));
                            setTimeout(() => insertar(), 0);
                          }}
                          style={{
                            background: isSel ? "#dbeafe" : "transparent",
                            cursor: "pointer",
                        ...
                        >
                          ...
                          <td style={{ padding: "6px 10px", color: "#475569" }}>
                            {el.grupo || grupoNombre(el.idgrupo)}
                          </td>
                          <td
                            style={{
                              padding: "6px 10px",
          ...
          >
            {cantSel !== 1 ? (
              ...
              <>
                    ...
                    <tbody>
                      {(detalle.productos || []).length === 0 && (
                        <tr>
                          ...
                          >
                            Este elemento no tiene productos.
                          </td>
                        </tr>
                      )}
                      {(detalle.productos || []).map((p, i) => {
                        const esCom =
                          p.es_comentario || p.idproducto === 118852;
                        if (esCom) {
                          return (
                            <tr
                              key={"cm-" + i}
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              style={{ padding: "4px 8px", color: "#171717" }}
                            >
                              {p.nombre || ""}
                            </td>
                            <td
                            ...
                            <td
                              style={{ padding: "4px 8px", color: "#737373" }}
                            >
                              {p.familia || ""}
                            </td>
                            <td
                ...
                >
                  {(detalle.productos || []).length} producto
                  {(detalle.productos || []).length !== 1 ? "s" : ""}
                </div>
              </>
            ) : null}
          </div>
        ...
        >
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button
              ...
              style={{
                padding: "7px 12px",
                borderRadius: 6,
                border: cantSel > 0 ? "1px solid #fca5a5" : "1px solid #d4d4d4",
                background: cantSel > 0 ? "#fef2f2" : "#f5f5f5",
                color: cantSel > 0 ? "#991b1b" : "#737373",
                cursor: cantSel > 0 ? "pointer" : "default",
                fontSize: 12,
              }}
            >
              <BtnContent
                icon={Trash2}
                iconColor={cantSel > 0 ? "#dc2626" : "#737373"}
              >
                Borrar Elemento{cantSel > 1 ? "s" : ""}
              </BtnContent>
            </button>
            <button
              onClick={() => {
                setGrupoNuevoId(null);
                setConfirmGrupo(true);
              }}
              disabled={cantSel === 0}
              style={{
                padding: "7px 12px",
                borderRadius: 6,
                border: cantSel > 0 ? "1px solid #cbd5e1" : "1px solid #d4d4d4",
                background: cantSel > 0 ? "#fff" : "#f5f5f5",
                color: cantSel > 0 ? "#171717" : "#737373",
                cursor: cantSel > 0 ? "pointer" : "default",
                fontSize: 12,
              }}
            >
              <BtnContent
                icon={FolderOpen}
                iconColor={cantSel > 0 ? "#475569" : "#737373"}
              >
                Cambiar grupo
              </BtnContent>
            </button>
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border:
                  cantSel === 1 ? "1px solid #cbd5e1" : "1px solid #d4d4d4",
                background: cantSel === 1 ? "#fff" : "#f5f5f5",
                color: cantSel === 1 ? "#171717" : "#737373",
                cursor: cantSel === 1 ? "pointer" : "default",
                fontSize: 12,
              }}
            >
              <BtnContent
                icon={Edit3}
                iconColor={cantSel === 1 ? "#475569" : "#737373"}
              >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              onClick={insertar}
              disabled={cantSel !== 1 || insertando}
              style={{
                padding: "7px 20px",
                borderRadius: 6,
                border:
                  cantSel === 1 && !insertando
                    ? "1px solid #16a34a"
                    : "1px solid #d4d4d4",
                background:
                  cantSel === 1 && !insertando ? "#dcfce7" : "#f5f5f5",
                color: cantSel === 1 && !insertando ? "#14532d" : "#737373",
                cursor: cantSel === 1 && !insertando ? "pointer" : "default",
                fontSize: 12,
                fontWeight: 600,
              }}
            >
              <BtnContent
                icon={insertando ? RefreshCw : Check}
                iconColor={cantSel === 1 && !insertando ? "#14532d" : "#737373"}
              >
                {insertando ? "Leyendo..." : "Leer Elemento"}
              </BtnContent>
            </button>
          </div>
        </div>
        ...
        {confirmBorrar && (
          ...
          >
            ...
            >
              ...
              >
                <Icon as={Trash2} size={18} color="#dc2626" /> ¿Borrar elemento
                {cantSel > 1 ? "s" : ""}?
              </h3>
              <p
                style={{
              ...
              >
                <strong>Atención:</strong> esta acción borra los elementos
                completos y todos sus productos asociados. No se puede deshacer.
              </div>
              <div
                style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}
              >
        ...
        {confirmGrupo && (
          <div
            ...
            onClick={() => !aplicandoGrupo && setConfirmGrupo(false)}
          >
            <div
              style={{
                background: "#fff",
            ...
            >
              ...
              >
                <Icon as={FolderOpen} size={18} color="#1e3a5f" /> Cambiar grupo
                al Elemento
              </h3>
              <p
                style={{
              ...
              >
                <button
                  onClick={() => setConfirmGrupo(false)}
                  disabled={aplicandoGrupo}
                  style={{
                    padding: "7px 14px",
                    borderRadius: 6,
                    border: "1px solid #d4d4d4",
                    background: "#fff",
                    color: "#171717",
                    cursor: aplicandoGrupo ? "default" : "pointer",
                    fontSize: 12,
                  }}
                >
                  <BtnContent icon={X}>Cancelar</BtnContent>
                ...
                <button
                  ...
                  disabled={aplicandoGrupo || grupoNuevoId == null}
                  style={{
                    padding: "7px 16px",
                    borderRadius: 6,
                    border:
                      grupoNuevoId != null && !aplicandoGrupo
                        ? "1px solid #16a34a"
                        : "1px solid #d4d4d4",
                    background:
                      grupoNuevoId != null && !aplicandoGrupo
                ...
                >
                  <BtnContent
                    icon={aplicandoGrupo ? RefreshCw : Check}
                    iconColor={
                      grupoNuevoId != null && !aplicandoGrupo
                        ? "#14532d"
                        : "#737373"
                  ...
                  >
                    {aplicandoGrupo ? "Aplicando..." : "Aplicar grupo"}
                  </BtnContent>
                </button>
              </div>
            </div>
        ...
        {editar && (
          <div
            ...
            onClick={() => !guardandoEditar && setEditar(null)}
          >
            <div
              style={{
                background: "#fff",
            ...
            >
              ...
              >
                <Icon as={Edit3} size={18} color="#1e3a5f" /> Cambiar nombre /
                descripción
              </h3>
              <label
                style={{
              ...
              <input
                value={editar.nombre}
                onChange={(e) =>
                  setEditar({ ...editar, nombre: e.target.value })
                }
                style={{
                  width: "100%",
                  padding: "7px 10px",
              ...
              <textarea
                value={editar.descripcion}
                onChange={(e) =>
                  setEditar({ ...editar, descripcion: e.target.value })
                }
                rows={4}
                style={{
                  width: "100%",
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    background: "#fff",
                    color: "#171717",
                    cursor: guardandoEditar ? "default" : "pointer",
                    fontSize: 12,
                  }}
                >
                  <BtnContent icon={X}>Cancelar</BtnContent>
                ...
                >
                  <BtnContent
                    icon={guardandoEditar ? RefreshCw : Save}
                    iconColor="#14532d"
                  >
...
function MenuItem({ item, onAction }) {
  ...
  return (
    ...
    >
      <Icon as={item.icon} size={14} color="#64748b" />
      <span>{item.label}</span>
    </div>
  );
}
    ...
    >
      <div
        style={{
          ...
          fontSize: 13,
          fontWeight: 500,
          color: open ? "#2563eb" : "#1e293b",
          borderBottom: open ? "2px solid #2563eb" : "2px solid transparent",
          userSelect: "none",
          whiteSpace: "nowrap",
          display: "inline-flex",
          alignItems: "center",
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Componente: <strong style={{ color: "#2563eb" }}>{action}</strong>
          </p>
        </div>
        <button
          onClick={onClose}
...
function DonutChart({ data, total, onHover, onLeave }) {
  ...
  const segs = data.map((item, i) => {
    ...
    const xi1 = cx + inner * Math.cos(angulo - sweep),
      yi1 = cy + inner * Math.sin(angulo - sweep);
    const xi2 = cx + inner * Math.cos(angulo),
      yi2 = cy + inner * Math.sin(angulo);
    ...
    >
      {segs.map((s, i) => (
        <path
          ...
          onMouseEnter={(e) =>
            onHover({ i, item: s.item, pct: s.pct, x: e.clientX, y: e.clientY })
          }
          onMouseMove={(e) =>
            onHover((t) => ({ ...t, x: e.clientX, y: e.clientY }))
          }
          onMouseLeave={onLeave}
        />
      ))}
      <text x={cx} y={cy - 8} textAnchor="middle" fontSize={12} fill="#64748b">
        Total
      </text>
      <text
        x={cx}
...
function parseSiemensRefs(textoOriginal) {
  ...
  const reConGuion =
    /\b\d[A-Z]{2}\d{4}(?:-\d(?:[A-Z]{1,2}(?:\d{1,2}(?:-\d(?:[A-Z]{1,2}(?:\d)?)?)?)?)?)?\b/g;
  let m;
...
function LeerPreciosPMDDialog({ dialog, onClose, setStatus, aplicarAFila }) {
  ...
  const obtenerYAplicar = async () => {
    ...
    try {
      const r = await fetch(
        `${API_LOCAL_URL}/precio?mlfb=${encodeURIComponent(ref)}`
      );
      ...
      if (dialog.rowIdx >= 0) {
        ...
        setStatus(
          `Datos de PMD aplicados a la fila ${dialog.rowIdx + 1} (${ref})`,
          "success"
        );
        onClose();
      } else {
        setStatus(
          `Datos recibidos pero no hay fila seleccionada para aplicar`,
          "info"
        );
      }
    } catch (e) {
  ...
  const guardarCSV = async () => {
    ...
    try {
      const r = await fetch(
        `${API_LOCAL_URL}/precio_csv?mlfb=${encodeURIComponent(ref)}`
      );
      if (!r.ok) throw new Error("HTTP " + r.status);
      const data = await r.json();
      if (data.error) {
        setStatus(
          `CSV generado con aviso: ${data.error} (${data.csv_path || ""})`,
          "warning"
        );
      } else {
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            onClick={guardarCSV}
            disabled={cargando || !referencia.trim()}
            style={{
              padding: "7px 14px",
              borderRadius: 6,
              border: "1px solid #cbd5e1",
              background: "#fff",
              color: "#171717",
              cursor: cargando || !referencia.trim() ? "default" : "pointer",
              fontSize: 12,
              fontWeight: 600,
            }}
          >
            <BtnContent
              icon={cargando && accion === "csv" ? RefreshCw : FileSpreadsheet}
              iconColor="#475569"
            >
              {cargando && accion === "csv"
                ? "Generando..."
          ...
          >
            <BtnContent
              icon={cargando && accion === "presupuesto" ? RefreshCw : Check}
              iconColor="#14532d"
            >
              {cargando && accion === "presupuesto"
                ? "Consultando..."
                : "datos de PMD a Presupuesto"}
            </BtnContent>
          </button>
        </div>
      </div>
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.ok) {
          setAviso({
            tipo: "ok",
            texto: `Login OK — bienvenido ${data.usuario}`,
          });
          ...
          setAviso({
            tipo: "error",
            texto: `Login KO — ${data.motivo || "credenciales incorrectas"}`,
          });
        }
      } else {
      ...
      setAviso({
        tipo: "error",
        texto: `No se pudo verificar: ${err.message}`,
      });
    }
    setVerificando(false);
  };
    ...
    >
      ...
      >
        ...
        >
          <label
            htmlFor="ld-user"
            style={{ fontSize: 11, color: "#525252", fontWeight: 500 }}
          >
            Usuario
          </label>
          <div style={{ position: "relative" }}>
        ...
        >
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && !verificando) submit();
              }}
              style={{
                width: "100%",
                padding: "8px 10px 8px 32px",
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border: "1px solid #16a34a",
              background: verificando ? "#f5f5f5" : "#dcfce7",
              color: "#14532d",
              cursor: verificando ? "default" : "pointer",
              fontSize: 12,
              fontWeight: 600,
          ...
          >
            <BtnContent
              icon={verificando ? RefreshCw : Check}
              iconColor="#14532d"
            >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Icon
            as={verificando ? RefreshCw : Check}
            size={14}
            color="#14532d"
          />
...
function SiemensRefsDialog({
  ...
  const refsAceptadasCelda = aceptadas.filter(
    (a) => a.rowId === celdaActual?.rowId && a.colKey === celdaActual?.colKey
  ).length;
  ...
  const buscarEnBD = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/productos/referencia/${encodeURIComponent(refActual.ref)}`
      );
      if (res.ok) {
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon as={Search} size={16} color="#171717" /> Referencia SIEMENS
            detectada
          </h3>
          <span style={{ fontSize: 11, color: "#737373" }}>
            Celda {posCelda + 1} de {totalCeldas} · Ref {posRef + 1} de{" "}
            {totalRefsCelda}
          </span>
        </div>
        ...
        >
          ...
          >
            Referencia detectada (normalizada en mayúsculas):
          </div>
          <div
            style={{
              fontSize: 18,
        ...
        {busqBD.existe !== null && (
          ...
          >
            {busqBD.existe ? (
              <>
                <strong>✓ Existe en BD</strong> — el producto con esta
                referencia está registrado.
              </>
            ) : (
              <>
                <strong>✗ No existe en BD</strong> — esta referencia no está
                registrada como producto.
              </>
            )}
          </div>
        )}
        ...
        >
          <button
            ...
            style={{
              ...
              background: "#fff",
              color: "#171717",
              cursor: busqBD.buscando ? "default" : "pointer",
              fontSize: 12,
            }}
          >
            <BtnContent
              icon={busqBD.buscando ? RefreshCw : Database}
              iconColor="#475569"
            >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            >
              <BtnContent icon={CornerDownLeft} iconColor="#475569">
                Saltar
              </BtnContent>
            </button>
            <button
...
export default function App() {
  ...
  const [rows, setRows] = useState(() => {
    ...
    return saved && Array.isArray(saved.rows) && saved.rows.length > 0
      ? saved.rows
      : INITIAL_ROWS;
  });
  ...
  useEffect(() => {
    const onPaste = (e) => {
      ...
      if (!selectedCell) {
        e.preventDefault();
        setStatus(
          "Para pegar, primero haz click en una celda del grid (verás un rectángulo azul)",
          "warning"
        );
        return;
      }
      ...
      setRows((rs) => {
        ...
        setTimeout(() => {
          if (isTabular) {
            setStatus(
              `Pegadas ${pegadas} celdas (${matrix.length} fila${
                matrix.length > 1 ? "s" : ""
              } × ${matrix[0].length} col${matrix[0].length > 1 ? "s" : ""})${
                omitidas > 0 ? `, ${omitidas} columnas calculadas omitidas` : ""
              }`,
              "success"
            );
          } else {
            setStatus(
              `Valor pegado en ${selectedCell.colKey} fila ${rowIdx + 1}`,
              "success"
            );
          }
        }, 0);
    ...
    const onCopy = (e) => {
      ...
      if (selectionRange) {
        const minRow = Math.min(
          selectionRange.startRowIdx,
          selectionRange.endRowIdx
        );
        ...
        const minCol = Math.min(
          selectionRange.startColIdx,
          selectionRange.endColIdx
        );
        ...
        for (let r = minRow; r <= maxRow; r++) {
          ...
          for (let c = minCol; c <= maxCol; c++) {
            ...
            else if (col.key === "costeposicion")
              val =
                (Number(row.preciocosteunitario) || 0) *
      ...
      if (selectionRange) {
        const rows_n =
          Math.abs(selectionRange.endRowIdx - selectionRange.startRowIdx) + 1;
        const cols_n =
          Math.abs(selectionRange.endColIdx - selectionRange.startColIdx) + 1;
        setStatus(
          `Copiado ${rows_n}×${cols_n} (${
            rows_n * cols_n
          } celdas) al portapapeles`,
          "success"
        );
      } else {
        setStatus(
          `Copiada celda ${selectedCell.colKey} al portapapeles`,
          "success"
        );
      }
    };
    ...
    (async () => {
      ...
      try {
        ...
        setPresupuesto((p) => {
          ...
          newP.numerocompleto = buildNumeroCompleto(
            codigoUsuario,
            newP.np,
            newP.anopresupuesto
          );
        ...
        setStatus(
          `Siguiente número asignado: ${buildNumeroCompleto(
            codigoUsuario,
            sig,
            ano
          )}`,
        ...
        if (cancel) return;
        setStatus(
          "No se ha podido obtener el siguiente número de la BD: " + e.message,
          "error"
        );
      }
    })();
  ...
  const setStatus = useCallback((text, type = "info") => {
    ...
    if (type === "success" || type === "error") {
      setTimeout(() => {
        setStatusMessage((s) =>
          s.timestamp === ts
            ? { text: "Listo", type: "info", timestamp: null }
            : s
        );
      }, 5000);
    }
  ...
  const getColWidth = (col) => {
    ...
    if (estructuraActiva && COLS_FIT.includes(col.key) && anchosAuto[col.key])
      return anchosAuto[col.key];
    return col.width;
  };
  ...
  const buscarPresupuestoEnBD = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/presupuestos/?busqueda=${encodeURIComponent(numComp)}`
      );
      if (!res.ok) throw new Error("Error " + res.status);
      const lista = await res.json();
      // Filtrar por numerocompleto exacto y revisión
      const enc = lista.find(
        (p) =>
  ...
  const guardarPresupuestoEnBD = async (idExistente) => {
    ...
    const body = {
      ...
      anopresupuesto:
        parseInt(presupuesto.anopresupuesto, 10) || getAnoFiscal(),
    ...
    const url = idExistente
      ? `${API_URL}/presupuestos/${idExistente}`
      : `${API_URL}/presupuestos/`;
  ...
  const handleAction = useCallback(
    (action, label) => {
      if (action === "Resumen") {
        setVista("resumen");
        setStatus("Mostrando resumen del presupuesto", "info");
        return;
      }
      if (action === "Ayuda") {
        setVista("ayuda");
        setStatus("Mostrando pantalla de ayuda", "info");
        return;
      }
      if (action === "Opciones") {
        setVista("opciones");
        setStatus("Configurando opciones", "info");
        return;
      }
      if (action === "AplicarEstructura") {
        setEstructuraActiva((e) => {
          setStatus(
            e ? "Estructura desactivada" : "Estructura aplicada al presupuesto",
            "success"
          );
          return !e;
        });
      ...
      if (action === "Imprimir") {
        ...
        try {
          ...
          setStatus(
            "Fichero Excel generado y descargado correctamente",
            "success"
          );
        } catch (err) {
          setStatus(
            "Error al generar el Excel: " + (err.message || err),
            "error"
          );
        }
        return;
      ...
      if (action === "FormatoSimpleQuote") {
        ...
        try {
          ...
          if (productos.length === 0) {
            setStatus(
              "No hay productos en el presupuesto para exportar",
              "error"
            );
            return;
          }
          const calcNetoUnit = (row) =>
            (Number(row.pvp) || 0) * (1 - (Number(row.dtoaplicado) || 0) / 100);
          ...
          setStatus(
            `Simple Quote generado con ${productos.length} producto${
              productos.length !== 1 ? "s" : ""
            }`,
            "success"
          );
          ...
          setStatus(
            "Error al generar Simple Quote: " + (err.message || err),
            "error"
          );
        }
        return;
      ...
      if (action === "Importar") {
        ...
        setStatus("Abriendo diálogo de importación", "info");
        return;
      }
      if (action === "AplicarDescuentos") {
        setShowDescuentos(true);
        setStatus("Abriendo gestor de descuentos por grupo descuento", "info");
        return;
      }
      if (action === "LeerPresupuestos") {
        setShowLeer(true);
        setStatus(
          "Cargando lista de presupuestos desde la base de datos...",
          "working"
        );
        return;
      }
      if (action === "GestionarClientes") {
        setShowClientes(true);
        setStatus(
          "Cargando lista de clientes desde la base de datos...",
          "working"
        );
        return;
      }
      if (action === "GestionarContactos") {
        setShowContactos(true);
        setStatus(
          "Cargando lista de contactos desde la base de datos...",
          "working"
        );
        return;
      }
      if (action === "SeleccionarCeldas") {
        setShowSeleccionar(true);
        return;
      }
      if (action === "FijarPrecioTotal") {
        setShowFijarPrecio("total");
        return;
      }
      if (action === "FijarPrecioCeldas") {
        setShowFijarPrecio("celdas");
        return;
      }
      if (action === "BorrarFilas") {
        setShowBorrarVacias(true);
        return;
      }
      if (action === "BorrarPresupuestoActual") {
        setConfirmBorrarPresup(true);
        return;
      }
      if (action === "ComprobarPresupuesto") {
        setStatus("Comparando con el presupuesto guardado en BD...", "working");
        buscarPresupuestoEnBD()
          .then((bd) => {
            if (!bd) {
            ...
            if (diffs.length === 0) {
              ...
              setStatus(
                "El presupuesto actual es idéntico al guardado en BD",
                "success"
              );
            } else {
              setComprobarDialog({ tipo: "diferentes", diffs, comparado: bd });
              setStatus(
                `Se han detectado ${diffs.length} diferencia${
                  diffs.length > 1 ? "s" : ""
                } con la BD`,
                "info"
              );
      ...
      if (action === "JuntarCeldas") {
        ...
        if (selectionRange) {
          ...
          const rowIni = Math.min(
            selectionRange.startRowIdx,
            selectionRange.endRowIdx
          );
          const rowFin = Math.max(
            selectionRange.startRowIdx,
            selectionRange.endRowIdx
          );
          ...
          const colIdx = COLUMNS.findIndex(
            (c) => c.key === selectedCell.colKey
          );
          if (rowIdx >= 0 && colIdx >= 0) {
            celdasSel.push({
              rowIdx,
              colIdx,
              key: selectedCell.colKey,
              value: rows[rowIdx][selectedCell.colKey],
            });
          }
        }
        ...
        if (celdasTexto.length < 2) {
          setStatus(
            "Esta función solo opera sobre celdas de texto (referencia, producto, descripción...). Selecciona al menos 2 celdas de texto.",
            "error"
          );
          return;
        }
        ...
        setStatus(
          `Texto unido en ${primera.key} (fila ${primera.rowIdx + 1}); ${
            celdasTexto.length - 1
          } celda${celdasTexto.length - 1 !== 1 ? "s" : ""} vaciada${
            celdasTexto.length - 1 !== 1 ? "s" : ""
          }`,
          "success"
        );
        return;
      ...
      if (action === "ConvertirEnComentario") {
        ...
        if (celdasSel.length === 0) {
          setStatus(
            "Selecciona al menos una celda para convertir en comentario",
            "error"
          );
          return;
        }
        ...
        const filasAfectadasIdx = [
          ...new Set(celdasSel.map((c) => c.rowIdx)),
        ].sort((a, b) => a - b);
        const idsAEliminar = new Set(
          filasAfectadasIdx.map((idx) => rows[idx].id)
        );
        ...
        setStatus(
          `Comentario creado uniendo ${
            celdasSel.filter((c) => String(c.value ?? "").trim() !== "").length
          } valor${celdasSel.length !== 1 ? "es" : ""} de ${
            filasAfectadasIdx.length
          } fila${filasAfectadasIdx.length !== 1 ? "s" : ""}`,
          "success"
        );
        return;
      }
      if (action === "GuardarPresupuesto") {
        if (!presupuesto.idcliente) {
          setStatus(
            "Selecciona un cliente antes de guardar el presupuesto",
            "error"
          );
          return;
        }
          ...
          .then(async (bd) => {
            if (!bd) {
              ...
              try {
                ...
                setStatus(
                  `Presupuesto guardado en BD (ID ${r.id}, ${r.lineas_insertadas} líneas)`,
                  "success"
                );
              } catch (e) {
            ...
            if (diffs.length === 0) {
              setStatus(
                "El presupuesto ya está guardado en BD sin cambios",
                "info"
              );
              setComprobarDialog({ tipo: "iguales", comparado: bd });
              return;
            ...
            setConfirmSobreescribir({
              idBd: bd.cabecera.id,
              diffs,
              lineasBd: bd.detalle.length,
            });
          })
          .catch((e) => {
      ...
      if (action === "GuardarElemento") {
        if (selectedRows.size === 0) {
          setStatus(
            "Selecciona al menos una fila con un producto para guardar como elemento",
            "error"
          );
          return;
        }
      ...
      if (action === "LeerElemento") {
        setShowLeerElem(true);
        return;
      }
      if (action === "JuntarDuplicados") {
        ...
        if (!selectionRange) {
          setStatus(
            "Selecciona un rango de filas para juntar duplicados",
            "error"
          );
          return;
        }
        ...
        const duplicados = [...grupos.entries()].filter(
          ([_, idxs]) => idxs.length > 1
        );
        if (duplicados.length === 0) {
          setStatus(
            "No se han encontrado referencias duplicadas en el rango seleccionado",
            "info"
          );
          return;
        }
        const totalFilasAfectadas = duplicados.reduce(
          (sum, [_, idxs]) => sum + idxs.length,
          0
        );
        ...
        (async () => {
          try {
            ...
            if (presupuesto.idcliente) {
              try {
                const r = await fetch(
                  `${API_URL}/clientes/${presupuesto.idcliente}`
                );
                if (r.ok) {
            ...
            if (!razonSocial)
              razonSocial = presupuesto.cliente || "(cliente no definido)";

            // Filtrar líneas de producto (PD, PE, E) con cantidad y referencia
            const productos = rows
              .filter((r) => ["PD", "PE", "E"].includes(r.naturaleza))
              .filter(
                (r) =>
                  String(r.referencia || "").trim() !== "" &&
                  Number(r.cantidad) > 0
            ...
            if (productos.length === 0) {
              setStatus(
                "No hay líneas de producto con referencia y cantidad en el presupuesto",
                "error"
              );
              return;
            }
            ...
            const lineaCab =
              "Cantidad".padStart(anchoCant) + sep + "Referencia";
            const lineaSep = "-".repeat(anchoCant) + sep + "-".repeat(20);
            const lineasTabla = filas
              .map((f) => f.cant.padStart(anchoCant) + sep + f.ref)
              .join("\n");
            const cuerpo = `Indicamos que el cliente es: ${razonSocial}\n\n${lineaCab}\n${lineaSep}\n${lineasTabla}\n`;

            const payload = {
              destinatario: "simplequote-rfq.industry@siemens.com",
              asunto:
                String(presupuesto.titulo || "").trim() ||
                "(presupuesto sin título)",
              cuerpo,
              cc: "josecarlos.demena@siemens.com",
              adjuntos: [],
            };
            ...
            setStatus(
              `SimpleQuote enviada con ${productos.length} línea${
                productos.length !== 1 ? "s" : ""
              }`,
              "success"
            );
            ...
            setStatus(
              `Error al enviar SimpleQuote: ${e.message}. Comprueba que la API local en localhost:8000 esté en marcha.`,
              "error"
            );
          }
        })();
      ...
      if (action === "HacerDamexE") {
        ...
        if (!presupuesto.idpaisclientefinal)
          faltan.push("País del cliente final");
        const clienteFinal = String(presupuesto.clientefinal || "").trim();
        if (!clienteFinal) faltan.push("Cliente final");
        if (faltan.length > 0) {
          setStatus(
            `No se puede ejecutar Damex E. Faltan: ${faltan.join(", ")}`,
            "error"
          );
          return;
        }
        // Resolver código ISO del país desde paisesList
        const paisObj = paisesList.find(
          (p) => p.id === presupuesto.idpaisclientefinal
        );
        if (!paisObj) {
          setStatus(
            `No se encuentra el país con id ${presupuesto.idpaisclientefinal} en la lista de países`,
            "error"
          );
          return;
        }
        ...
        (async () => {
          try {
            ...
            if (resultado === "NN") {
              setStatus(
                `Damex E completado correctamente (${resultado})`,
                "success"
              );
            } else {
              setStatus(
                `Damex E ha fallado: resultado "${
                  resultado || "(vacío)"
                }" (esperado "NN")`,
                "error"
              );
      ...
      if (action === "LeerPreciosPMD") {
        ...
        if (selectedCell) {
          ...
          if (rowIdxObjetivo >= 0)
            refInicial = String(rows[rowIdxObjetivo].referencia || "").trim();
        }
      ...
      if (action === "CalcularDescuento") {
        ...
        if (selectionRange) {
          ...
          if (rowIdx >= 0)
            celdas.push({
              rowIdx,
              key: selectedCell.colKey,
              valor: rows[rowIdx][selectedCell.colKey],
            });
        }
        if (celdas.length === 0) {
          setStatus(
            "Selecciona al menos una celda con un valor numérico para calcular el descuento",
            "error"
          );
          return;
        }
        ...
        if (objetivoPorFila.size === 0) {
          setStatus(
            "Las celdas seleccionadas no contienen valores numéricos válidos",
            "error"
          );
          return;
        }
        ...
        setRows((rs) => {
          ...
          objetivoPorFila.forEach((objetivo, rowIdx) => {
            ...
            if (pvp <= 0) {
              omitidas++;
              detalles.push(
                `Fila ${rowIdx + 1}: PVP es 0 o negativo, no se puede calcular`
              );
              return;
            }
            ...
            next[rowIdx] = {
              ...fila,
              dtoaplicado: Math.round(dto * 100) / 100,
            };
            aplicadas++;
          });
          return next;
        ...
        if (aplicadas > 0 && omitidas === 0) {
          setStatus(
            `Descuento recalculado en ${aplicadas} fila${
              aplicadas !== 1 ? "s" : ""
            }`,
            "success"
          );
          ...
          setStatus(
            `${aplicadas} fila${aplicadas !== 1 ? "s" : ""} ajustada${
              aplicadas !== 1 ? "s" : ""
            }, ${omitidas} omitida${omitidas !== 1 ? "s" : ""}${
              detalles.length ? ": " + detalles.slice(0, 2).join("; ") : ""
            }`,
            "warning"
          );
        } else {
          setStatus(
            `No se pudo calcular: ${detalles[0] || "ninguna fila válida"}`,
            "error"
          );
        }
        return;
      ...
      if (action === "BuscarReferencia") {
        ...
        if (selectionRange) {
          ...
          rows.forEach((r) => {
            if (selectedRows.has(r.id)) {
              ["referencia", "nombre", "descripcion"].forEach((k) =>
                celdasObjetivo.push({ rowId: r.id, colKey: k })
              );
            }
          });
        }
        if (celdasObjetivo.length === 0) {
          setStatus(
            "Selecciona al menos una celda o fila para buscar referencias SIEMENS",
            "error"
          );
          return;
        }
        ...
        if (celdasTrabajadas.length === 0) {
          setStatus(
            "No se encontraron referencias SIEMENS en las celdas seleccionadas",
            "info"
          );
          return;
        }
        const totalRefs = celdasTrabajadas.reduce(
          (sum, c) => sum + c.refs.length,
          0
        );
        setStatus(
          `Encontradas ${totalRefs} referencia${
            totalRefs !== 1 ? "s" : ""
          } SIEMENS en ${celdasTrabajadas.length} celda${
            celdasTrabajadas.length !== 1 ? "s" : ""
          }`,
          "info"
        );
      ...
      if (action === "GuardarProducto") {
        ...
        if (selectionRange) {
          const rowIni = Math.min(selectionRange.row1, selectionRange.row2);
          const rowFin = Math.max(selectionRange.row1, selectionRange.row2);
          for (let r = rowIni; r <= rowFin; r++) {
            const row = rows[r];
        ...
        if (idsProcesar.size === 0) {
          setStatus(
            "Selecciona al menos una fila para guardar como producto",
            "error"
          );
          return;
        }
        ...
        (async () => {
          setStatus(
            `Guardando ${filasObjetivo.length} producto${
              filasObjetivo.length !== 1 ? "s" : ""
            }...`,
            "working"
          );
          ...
          for (const fila of filasObjetivo) {
            ...
            if (!ref || pvp <= 0 || !descripcion || !grupoCod) {
              ...
              detalles.push(
                `✗ ${
                  ref || "(sin ref)"
                }: faltan campos obligatorios (ref, pvp, descripción, grupodescuento)`
              );
              continue;
            }
            ...
            if (idGrupo === null) {
              saltadosGrupo++;
              detalles.push(
                `✗ ${ref}: grupo descuento "${grupoCod}" no existe en BD`
              );
              continue;
            }
            ...
            if (!isNaN(preciocoste) && preciocoste > 0)
              payload.preciocoste = preciocoste;

            // Comprobar si ya existe por referencia
            try {
              const rExist = await fetch(
                `${API_URL}/productos/referencia/${encodeURIComponent(ref)}`
              );
              if (rExist.status === 404) {
                ...
                const refDevuelta =
                  data && data.referencia
                    ? String(data.referencia).trim().toUpperCase()
                    : "";
                if (data && data.id && refDevuelta === ref.toUpperCase()) {
                  saltadosExistentes++;
                  detalles.push(
                    `⊘ ${ref}: ya existe en BD (id ${data.id}), se omite`
                  );
                  continue;
                }
                // 2xx pero no es un producto válido → seguimos al INSERT (lo trataremos como inexistente)
                detalles.push(
                  `! ${ref}: respuesta 2xx inesperada (sin id/ref), se intenta crear igualmente`
                );
              } else {
              ...
              errores++;
              detalles.push(
                `✗ ${ref}: error comprobando existencia (${e.message})`
              );
              continue;
            }
          ...
          const partes = [];
          if (creados > 0)
            partes.push(`${creados} creado${creados !== 1 ? "s" : ""}`);
          if (saltadosExistentes > 0)
            partes.push(`${saltadosExistentes} ya existían`);
          if (saltadosFaltan > 0)
            partes.push(`${saltadosFaltan} sin campos obligatorios`);
          if (saltadosGrupo > 0)
            partes.push(`${saltadosGrupo} con grupo inválido`);
          if (errores > 0) partes.push(`${errores} con error`);
          const resumen = partes.join(", ");
          const tipo = errores > 0 ? "error" : creados > 0 ? "success" : "info";
          setStatus(
            `Guardar Producto: ${resumen || "nada que procesar"}`,
            tipo
          );
      ...
      if (action === "LeerProducto") {
        setShowLeerProducto(true);
        return;
      }
      if (action === "BuscarDatosProductos") {
        ...
        const filasConRef = rows.filter(
          (r) =>
            idsProcesar.has(r.id) &&
            r.referencia &&
            String(r.referencia).trim() !== ""
        );
        ...
        if (filasConRef.length === 0) {
          setStatus(
            "Ninguna de las filas seleccionadas tiene referencia",
            "error"
          );
          return;
        }
        ...
        setStatus(
          `Buscando ${filasConRef.length} referencia${
            filasConRef.length !== 1 ? "s" : ""
          } en BD...`,
          "working"
        );
        ...
        (async () => {
          ...
          for (const row of filasConRef) {
            ...
            try {
              ...
              const cantidadFinal =
                isNaN(cantNum) || cantNum <= 0 ? 1 : cantNum;
              actualizaciones.set(row.id, {
                ...
                preciocosteunitario:
                  Number(prod.preciocoste) || row.preciocosteunitario || 0,
                idposicion: prod.id || "",
              });
              ok++;
            } catch (e) {
          ...
          if (actualizaciones.size > 0) {
            setRows((rs) =>
              rs.map((r) =>
                actualizaciones.has(r.id)
                  ? { ...r, ...actualizaciones.get(r.id) }
                  : r
              )
            );
          }
          ...
          if (noEncontradas > 0)
            partes.push(
              `${noEncontradas} no encontrado${noEncontradas !== 1 ? "s" : ""}`
            );
          if (errores > 0)
            partes.push(`${errores} error${errores !== 1 ? "es" : ""}`);
          const tipo =
            errores > 0 ? "error" : noEncontradas > 0 ? "info" : "success";
  ...
  const startEdit = (rowId, colKey, val) => {
  ...
  const commitEdit = (override) => {
    if (!editingCell) return;
    const rawValue =
      override !== undefined && override !== null ? override : editValue;
    setRows((r) =>
      r.map((row) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Icon as={HelpCircle} size={18} color="#171717" /> Ayuda — Manual de
            uso
          </span>
          <span style={{ color: "#737373", fontSize: 12 }}>
            v1.62.2 (1 Junio 2026)
          </span>
        </div>
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          {/* ÁRBOL IZQUIERDA */}
            ...
            >
              <section id="ayuda-intro">
                ...
                <p style={{ color: "#475569", lineHeight: 1.8, margin: 0 }}>
                  Aplicación para crear y gestionar{" "}
                  <strong>presupuestos comerciales</strong>. Cada presupuesto
                  tiene una cabecera con datos generales (ID, número, revisión,
                  título, cliente) y una grid de líneas con productos,
                  elementos, apartados y subtotales. Los importes neto, coste y
                  margen se calculan automáticamente. La numeración de apartados
                  se genera sola al activar la estructura, los subtotales
                  (S1-S4) calculan la suma y descuento medio de su apartado, y
                  los productos se autocompletan al teclear referencias gracias
                  a la conexión con la base de datos. La aplicación permite
                  importar/exportar Excel, leer presupuestos y elementos
                  guardados, gestionar clientes y aplicar descuentos por
                  subfamilia o fijar precios totales con reparto proporcional al
                  neto.
                </p>
              </section>
              ...
              <section id="ayuda-novedades">
                ...
                >
                  Resumen de los cambios más recientes incorporados a la
                  aplicación, agrupados por área.
                </p>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Icon as={Palette} size={16} color="#16a34a" /> Aplicar
                      estructura
                    </h3>
                    <ul
                      style={{
                    ...
                    >
                      <li>
                        <strong>Numeración jerárquica automática</strong> de
                        apartados (T1 → 1, T2 → 1.1, T3 → 1.1.1, etc.), sin
                        ceros (mínimo 1 en cada nivel).
                      </li>
                      <li>
                        Anchos de columna{" "}
                        <strong>auto-ajustados al contenido</strong>{" "}
                        (representación, naturaleza, cantidad, PVP, dto, neto
                        unit., neto pos., apartado).
                      </li>
                      <li>
                        <strong>Multilínea automático</strong> en Referencia,
                        Producto y Descripción si no caben en una línea. Tooltip
                        con el contenido completo.
                      </li>
                      <li>
                        Importes formateados como <strong>moneda con €</strong>{" "}
                        y descuentos con <strong>2 decimales + %</strong>.
                      </li>
                      <li>
                        <strong>Subtotales S1-S4 y total TT</strong> calculados
                        automáticamente. Estilos selectivos: títulos centrados,
                        subtotales/total justificados a la izquierda.
                      </li>
                      <li>
                        El campo nombre de{" "}
                        <strong>
                          S1-S4 se rellena con "TOTAL &lt;texto del Tn
                          anterior&gt;"
                        </strong>
                        ; el de{" "}
                        <strong>
                          TT con "TOTAL &lt;título del presupuesto&gt;"
                        </strong>
                        . Persiste al desactivar estructura.
                      </li>
                      <li>
                        <strong>Importes negativos en rojo</strong> (PVP, Dto,
                        netos, coste, neto2) cuando estructura activa.
                      </li>
                      <li>
                        Al <strong>desactivar</strong> estructura los formatos
                        quedan tal cual (anchos, multilínea, etiquetas). Al
                        volver a activar se recalcula todo.
                      </li>
                    </ul>
                  </div>
                  ...
                  >
                    ...
                    >
                      <Icon as={Grid3x3} size={16} color="#7c3aed" /> Grid de
                      líneas
                    </h3>
                    <ul
                      style={{
                    ...
                    >
                      <li>
                        <strong>Redimensionado a mano</strong> (estilo Excel)
                        arrastrando el borde derecho de la cabecera o el borde
                        inferior del número de fila.
                      </li>
                      <li>
                        Los <strong>anchos y altos persisten</strong> al
                        desactivar estructura. Activarla recalcula los anchos
                        auto (sobreescribiendo los manuales hasta el siguiente
                        cambio).
                      </li>
                      <li>
                        Edición de celdas de texto largo (Referencia / Producto
                        / Descripción) con <strong>textarea flotante</strong>{" "}
                        que se autoajusta al contenido (filas variables).
                      </li>
                      <li>
                        Selección, copiar/pegar y borrar{" "}
                        <strong>funcionan dentro del editor</strong> (no hay
                        interferencia con los handlers globales del grid).
                      </li>
                      <li>
                        Sin <strong>límite de 50 caracteres</strong> en la
                        columna Producto (admite títulos y comentarios largos).
                      </li>
                      <li>
                        Naturaleza y Representación se pueden{" "}
                        <strong>editar a mano</strong> (con filtrado y
                        validación contra la lista permitida).
                      </li>
                      <li>
                        Presupuestos nuevos arrancan con{" "}
                        <strong>50 filas vacías</strong> en vez de 10.
                      </li>
                      <li>
                        Botones <strong>Copiar filas / Pegar filas</strong> en
                        la barra del grid: copia las filas marcadas, pega a
                        partir de la celda activa (rellena vacías o inserta si
                        están ocupadas).
                      </li>
                      <li>
                        Pegar números con formato europeo (
                        <code>3.948,45 €</code>) en celdas numéricas se
                  ...
                  >
                    ...
                    >
                      <Icon as={MessageSquare} size={16} color="#dc2626" />{" "}
                      Celdas
                    </h3>
                    <ul
                      style={{
                    ...
                    >
                      <li>
                        <strong>Juntar celdas en una</strong>: concatena el
                        texto de varias celdas de texto en la primera, vaciando
                        las demás.
                      </li>
                      <li>
                        <strong>Convertir selección en comentario</strong>: une
                        las celdas seleccionadas con " - " en una nueva fila CM
                  ...
                  >
                    ...
                    >
                      <li>
                        <strong>Calcular descuento</strong>: para cada celda
                        numérica seleccionada, ajusta el dto. de su fila para
                        que el neto unitario coincida con el valor objetivo.
                        Admite descuentos negativos (recargo).
                      </li>
                      <li>
                        <strong>Aplicar descuentos por Grupo Descuento</strong>:
                        agrupa por <code>grupo + dto actual</code> (filas
                        separadas por descuento). Muestra <strong>DGL1</strong>{" "}
                        y <strong>DGL2</strong> del grupo; resalta el dto. en{" "}
                        <strong>amarillo si supera DGL1</strong> y en{" "}
                        <strong>rojo si supera DGL2</strong>.
                      </li>
                    </ul>
                  </div>
                  ...
                  >
                    ...
                    >
                      <li>
                        <strong>Buscar datos por Referencia</strong> rellena
                        además <code>naturaleza = PD</code> y fuerza{" "}
                        <code>cantidad ≥ 1</code>.
                      </li>
                      <li>
                        <strong>Leer Producto</strong> inserta en la fila de la{" "}
                        <strong>celda activa</strong>: si está vacía la rellena;
                        si tiene contenido inserta una fila nueva encima.
                      </li>
                      <li>
                        <strong>Juntar productos duplicados</strong>: suma
                        cantidades en la primera fila del grupo (dejar a 0 o
                        borrar duplicadas).
                      </li>
                    </ul>
                  </div>
                  ...
                  >
                    ...
                    >
                      <Icon as={Wrench} size={16} color="#7c3aed" />{" "}
                      Mantenimiento BD
                    </h3>
                    <ul
                      style={{
                    ...
                    >
                      <li>
                        <strong>Mantenimiento BD Familia / Subfamilia</strong>:
                        editar y borrar (con validación de uso antes de borrar).
                      </li>
                      <li>
                        <strong>Mantenimiento Grupos Descuento</strong>: importa
                        Excel con drag&drop. Reconoce las columnas{" "}
                        <strong>DGL1</strong> y <strong>DGL2</strong>. Recuadro
                        de mapeo con tags verde/rojo y errores. Selector SÍ/NO
                        para sobreescribir existentes.
                      </li>
                      <li>
                        <strong>Actualizar Tarifas</strong>: matching de
                        cabeceras por prefijo del nombre real del campo BD (sin
                        aliases). Ej: <code>pla → plazoentrega</code>,{" "}
                        <code>precio → preciocoste</code>,{" "}
                        <code>fecha → ambigua</code>. Procesa todas las columnas
                        reconocidas (pvp_l1, plazoentrega, etc.).
                      </li>
                    </ul>
                  </div>
                  ...
                  >
                    ...
                    >
                      <Icon as={User} size={16} color="#2563eb" /> Sesión,
                      ventana y app
                    </h3>
                    <ul
                      style={{
                    ...
                    >
                      <li>
                        Pantalla de bienvenida con login real contra el backend
                        (mensaje OK/KO).
                      </li>
                      <li>
                        <strong>Pastilla de usuario</strong> en la barra
                        superior con menú Iniciar / Cerrar sesión.
                      </li>
                      <li>
                        El diálogo de Iniciar sesión también permite cambiar de
                        usuario sin salir.
                      </li>
                      <li>
                        Eliminado el <strong>scroll global</strong> de la
                        página: la ventana se dimensiona automáticamente sin
                        barra extra.
                      </li>
                    </ul>
                  </div>
              ...
              <section id="ayuda-cabecera">
                ...
                {[
                  {
                    campo: "ID",
                    desc: "Identificador único generado por la BD. No modificable.",
                  },
                  {
                    campo: "Nº Completo",
                    desc: "Solo lectura. Se construye automáticamente como CÓDIGO-NÚMERO-AÑO (ej. CDM-123-2026), donde el código viene del usuario activo. Se recalcula al cambiar el Nº o el Año.",
                  },
                  {
                    campo: "Nº",
                    desc: "Número de presupuesto. Al crear uno nuevo se asigna automáticamente como (max + 1) de los presupuestos del año actual.",
                  },
                  { campo: "Rev.", desc: "Número de revisión. Empieza en 0." },
                  {
                    campo: "Año",
                    desc: "Año fiscal del presupuesto. Por defecto el actual (año fiscal va del 1 octubre del año N-1 al 30 septiembre del año N). Se puede editar a mano para registrar presupuestos de años anteriores.",
                  },
                  { campo: "Título", desc: "Descripción breve del proyecto." },
                  {
                    campo: "Cliente",
                    desc: "Nombre del cliente destinatario.",
                  },
                ].map((r, i) => (
                  <div
                    key={i}
              ...
              <section id="ayuda-columnas">
                ...
                <p style={{ color: "#475569", marginBottom: 10, fontSize: 12 }}>
                  Doble click para editar. Las columnas en azul son calculadas
                  automáticamente.
                </p>
                {[
                  {
                    col: "Repres.",
                    desc: "Seleccionable: TP (Total Posición), Opcional (no suma al total), Conf (Confirmar). Puede dejarse vacío.",
                  },
                  {
                    col: "Nat.",
                    desc: "Naturaleza de la línea: PD/PE (Productos), E (Elemento), T1-T4 (Títulos), S1-S4 (Subtotales), CM (Comentario), TT (Total), VERDE/GRIS (líneas resaltadas). Obligatorio.",
                  },
                  {
                    col: "Apartado",
                    desc: "Numeración jerárquica automática al activar Estructura. T1→1, sus PD→1.1/1.2, T2→1.3...",
                  },
                  { col: "Cant.", desc: "Número de unidades." },
                  {
                    col: "Referencia",
                    desc: "Código de referencia del fabricante. Editable con autocompletado conectado a BD.",
                  },
                  {
                    col: "Producto",
                    desc: "Nombre del producto, elemento, título o texto del comentario (en filas CM).",
                  },
                  {
                    col: "PVP Unitario",
                    desc: "Precio de tarifa oficial unitario. Con Estructura activa: formato 1.234,56 €.",
                  },
                  {
                    col: "Dto. %",
                    desc: "Descuento aplicado. En S1-S4 muestra el descuento medio ponderado del apartado.",
                  },
                  {
                    col: "Neto Unitario",
                    desc: "Calculado: PVP × (1 - Dto% / 100). Con Estructura activa: formato 1.234,56 €.",
                  },
                  {
                    col: "Neto Posición",
                    desc: "Calculado: Neto Unitario × Cantidad. En S1-S4 muestra la suma del apartado. Con Estructura activa: formato 1.234,56 €.",
                  },
                  {
                    col: "Descripción",
                    desc: "Texto descriptivo adicional. Wrap automático (varias líneas).",
                  },
                  {
                    col: "Grupo Descuento",
                    desc: "Código del grupo descuento del producto (ej. GD123). Al pasar el ratón muestra la descripción completa del grupo.",
                  },
                  {
                    col: "Familia",
                    desc: "Familia del producto (deducida de la subfamilia).",
                  },
                  {
                    col: "SubFamilia",
                    desc: "Subfamilia del producto. Al pasar el ratón muestra la descripción completa.",
                  },
                  {
                    col: "Coste Unit. (GA)",
                    desc: "Precio de coste unitario (precio GA). Con Estructura activa: formato 1.234,56 €.",
                  },
                  {
                    col: "Coste Posición",
                    desc: "Calculado: Coste Unitario × Cantidad. Con Estructura activa: formato 1.234,56 €.",
                  },
                  {
                    col: "Margen %",
                    desc: "Calculado: (Neto Pos. - Coste Pos.) / Neto Pos. × 100.",
                  },
                  {
                    col: "Id Posición",
                    desc: "Identificador del producto en BD (idproducto). Útil para trazabilidad.",
                  },
                  {
                    col: "Imagen",
                    desc: "Ruta o referencia a la imagen del producto.",
                  },
                  {
                    col: "Neto Unit. 2",
                    desc: "Precio neto alternativo (segunda tarifa). Con Estructura activa: formato 1.234,56 €.",
                  },
                ].map((r, i) => (
                  <div
                    key={i}
              ...
              <section id="ayuda-acciones">
                ...
                {[
                  {
                    accion: "Nueva fila",
                    desc: "Sin selección → añade al final. Con filas seleccionadas → inserta delante de la primera seleccionada, empujando el resto hacia abajo.",
                  },
                  {
                    accion: "Campo numérico de filas",
                    desc: "Indica cuántas filas insertar (por defecto 1, máximo 100).",
                  },
                  {
                    accion: "Borrar seleccionadas",
                    desc: "Borra filas con checkbox marcado. Pide confirmación. Deshabilitado sin selección.",
                  },
                  {
                    accion: "Checkbox de fila",
                    desc: "Selecciona/deselecciona una fila. Se pueden marcar varias.",
                  },
                  {
                    accion: "Checkbox cabecera",
                    desc: "Selecciona o deselecciona todas las filas.",
                  },
                  {
                    accion: "Click en celda",
                    desc: "Selecciona la celda (rectángulo azul). Lista para Ctrl+C.",
                  },
                  {
                    accion: "Arrastrar con el ratón",
                    desc: "Selecciona un rango rectangular de celdas. Si arrastras hasta el borde del grid se hace auto-scroll en las 4 direcciones (estilo Excel).",
                  },
                  {
                    accion: "Doble click en celda",
                    desc: "Activa edición. Repres. y Nat. abren desplegable. Referencia abre autocompletado.",
                  },
                  {
                    accion: "Enter / clic fuera",
                    desc: "Confirma la edición.",
                  },
                  { accion: "Escape", desc: "Cancela la edición sin guardar." },
                  {
                    accion: "Ctrl+C",
                    desc: "Copia al portapapeles la celda o rango seleccionado. Las columnas calculadas exportan su valor calculado actual. Formato TSV compatible con Excel.",
                  },
                  {
                    accion: "Ctrl+V",
                    desc: "Pega desde el portapapeles. Si el contenido es tabular (de Excel u otra app) se pega en una matriz desde la celda actual hacia abajo y la derecha. Las columnas calculadas se omiten.",
                  },
                  {
                    accion: "Selector Larga/Corta",
                    desc: 'En la cabecera del presupuesto. "Larga" muestra descripciones multilínea. "Corta" limita las celdas a una línea con tooltip al pasar el ratón para ver el texto completo.',
                  },
                ].map((r, i) => (
                  <div
                    key={i}
              ...
              <section id="ayuda-statusbar">
                ...
                >
                  En la parte inferior de la pantalla hay una barra que muestra
                  mensajes de las acciones que ejecutas. Cambia de color según
                  el tipo de mensaje:
                </p>
                {[
                  {
                    tipo: "Información (negro)",
                    color: "#0f172a",
                    desc: "Estados generales: 'Listo', 'Mostrando resumen', etc.",
                  },
                  {
                    tipo: "Trabajando (azul)",
                    color: "#1e40af",
                    desc: "Operación en curso: 'Cargando lista de presupuestos...', 'Generando Excel...'",
                  },
                  {
                    tipo: "Éxito (verde)",
                    color: "#14532d",
                    desc: "Operación completada. Se auto-borra a los 5 segundos.",
                  },
                  {
                    tipo: "Error (rojo)",
                    color: "#7f1d1d",
                    desc: "Error en la operación. Se auto-borra a los 5 segundos.",
                  },
                ].map((r, i) => (
                  <div
                    key={i}
              ...
              <section id="ayuda-autoref">
                ...
                >
                  Al editar la columna <strong>Referencia</strong> con doble
                  click, se activa el autocompletado conectado a la base de
                  datos:
                </p>
                <ul
                  style={{
                ...
                >
                  <li>
                    Al teclear <strong>2 o más caracteres</strong> aparece un
                    tooltip con hasta 5 sugerencias mostrando referencia, nombre
                    y PVP.
                  </li>
                  <li>
                    Navega con las flechas <code>↑</code>/<code>↓</code> y
                    selecciona con <code>Enter</code>.
                  </li>
                  <li>
                    Al confirmar (Enter, click en sugerencia o salir del campo)
                    se buscan los datos completos del producto.
                  </li>
                  <li>
                    Si la referencia existe en BD, se rellenan automáticamente:{" "}
                    <strong>
                      nombre, descripción, PVP, familia, subfamilia y descuento
                    </strong>{" "}
                    (de la subfamilia, o de la familia si no hay).
                  </li>
                  <li>
                    Si la referencia no existe, se guarda solo el texto sin
                    sobrescribir nada (te deja crear referencias nuevas a mano).
                  </li>
                  <li>
                    <code>Escape</code> cancela la edición.
                  </li>
                </ul>
              </section>
              ...
              {[
                {
                  id: "m-presupuesto",
                  titulo: "Menú Presupuesto",
                  color: "#171717",
                  items: [
                    {
                      op: "Gestionar",
                      desc: "Abre el gestor para crear, abrir, copiar o eliminar presupuestos.",
                    },
                    {
                      op: "Leer Presupuestos",
                      desc: "Abre un diálogo con la lista de presupuestos guardados en la BD. Permite buscar por título, cliente o número, y cargar uno (machacando el presupuesto actual).",
                    },
                    {
                      op: "Importar",
                      desc: "Importa filas desde un Excel (fichero o copiar/pegar). Detecta automáticamente las columnas por su nombre (acepta abreviaturas como Ref, Can, PVP, Dto). Si encuentra columnas ambiguas avisa con error. El Neto Unitario tiene prioridad sobre el PVP.",
                    },
                    {
                      op: "Exportar",
                      desc: "Exporta el presupuesto a Excel con todos los datos crudos (21 columnas, una cabecera por fila, fila TOTAL al final). Solo exporta filas de producto (PD/PE/E).",
                    },
                    {
                      op: "Imprimir",
                      desc: "Genera un Excel formato Siemens listo para enviar al cliente. Solo muestra las columnas relevantes (Apartado, Cantidad, Referencia, Producto, Neto Unit., Neto Pos., Descripción) y aplica sangrías por nivel. Para que tenga colores instala xlsx-js-style y cambia el import.",
                    },
                    {
                      op: "Resumen",
                      desc: "Muestra el desglose por Familia/SubFamilia con tabla y gráfico donut interactivo.",
                    },
                    {
                      op: "Aplicar Estructura",
                      desc: "Activa/desactiva el modo estructura: colorea filas según naturaleza (T1/T2/T3/T4 azules, S1-S4 oscuros, CM amarillo, VERDE/GRIS), oculta celdas irrelevantes en títulos/subtotales/comentarios, calcula la numeración jerárquica de apartados automáticamente (T1→1, productos→1.1/1.2, T2→1.3, sus productos→1.3.1...) y en S1-S4 calcula la suma del apartado y el descuento medio.",
                    },
                    {
                      op: "Borrar Presupuesto actual",
                      desc: "Borra todas las líneas dejando la cabecera vacía.",
                    },
                    {
                      op: "Comparar Presupuestos",
                      desc: "Compara dos presupuestos línea a línea.",
                    },
                  ],
                },
                {
                  ...
                  items: [
                    {
                      op: "Comprobar Celda",
                      desc: "Comprueba el formato de la celda seleccionada.",
                    },
                    {
                      op: "Borrar filas vacías",
                      desc: "Abre un diálogo donde seleccionas una columna. Se borran las filas marcadas con checkbox que tengan esa columna vacía. Pide confirmación indicando cuántas se borrarán.",
                    },
                    {
                      op: "Juntar Duplicadas",
                      desc: "Agrupa líneas con la misma referencia sumando cantidades.",
                    },
                    {
                      op: "Ver Familia",
                      desc: "Muestra la familia del producto seleccionado.",
                    },
                    {
                      op: "Borrar filas con 0",
                      desc: "Elimina líneas con cantidad o importe cero.",
                    },
                    {
                      op: "Seleccionar Celdas",
                      desc: "Abre un diálogo para marcar el checkbox de un rango de filas (desde fila X hasta fila Y).",
                    },
                    {
                      op: "Convertir fila en comentario",
                      desc: "Cambia la naturaleza de la fila a CM.",
                    },
                  ],
                },
                {
                  id: "m-elementos",
                  titulo: "Menú Elementos",
                  color: "#171717",
                  items: [
                    {
                      op: "Guardar Elemento",
                      desc: "Toma las filas seleccionadas con checkbox y las guarda en BD como elemento reutilizable. Pide nombre y descripción. Avisa si alguna referencia no existe en BD.",
                    },
                    {
                      op: "Leer Elemento",
                      desc: "Abre un diálogo con la lista de elementos guardados. Al insertarlo, añade una fila de comentario (CM) con el nombre del elemento y debajo todas las filas de sus productos. Se inserta al final o delante de la primera fila seleccionada.",
                    },
                    {
                      op: "Buscar Elemento",
                      desc: "Buscador por nombre o grupo.",
                    },
                    {
                      op: "Borrar Elemento",
                    ...
                    {
                      op: "Crear Elemento en Excel",
                      desc: "Crea la estructura de un nuevo elemento en la grid.",
                    },
                    {
                      op: "Quitar Elemento de Excel",
                      desc: "Desvincula las líneas de su elemento.",
                    },
                    {
                      op: "Poner Título a Elemento",
                      desc: "Asigna o cambia el título de un elemento.",
                    },
                  ],
                },
                {
                  id: "m-productos",
                  titulo: "Menú Productos",
                  color: "#171717",
                  items: [
                    {
                      op: "Añadir Producto a BD",
                      desc: "Da de alta un producto con referencia, PVP y familia.",
                    },
                    {
                      op: "Leer Producto",
                      desc: "Inserta el producto seleccionado en el buscador en la fila actual.",
                    },
                    {
                      op: "Buscar datos por Referencia",
                      desc: "Rellena la fila con los datos del producto cuya referencia coincide.",
                    },
                    {
                      op: "Buscar Producto",
                      desc: "Buscador con filtros por referencia, descripción, familia y subfamilia.",
                    },
                    {
                      op: "Precio Neto",
                      desc: "Consulta el precio neto del producto según la tarifa activa.",
                    },
                    {
                      op: "Buscar Netos",
                      desc: "Muestra precios netos de todos los productos.",
                    },
                    {
                      op: "Quitar caracteres Referencia",
                      desc: "Elimina espacios y caracteres no válidos de la referencia.",
                    },
                    {
                      op: "Quitar Saltos de línea",
                      desc: "Limpia saltos de línea de la columna descripción.",
                    },
                    {
                      op: "Crear Fichero para PMD",
                      desc: "Genera fichero de exportación compatible con PMD Siemens.",
                    },
                    {
                      op: "Poner Costes desde fichero",
                      desc: "Importa precios de coste desde un fichero externo.",
                    },
                    {
                      op: "Poner Costes desde BD",
                      desc: "Rellena Coste Unitario con el precio guardado en BD.",
                    },
                    {
                      op: "Poner Costes de PMD",
                      desc: "Importa costes desde fichero PMD Siemens.",
                    },
                    {
                      op: "Buscar Referencia SIEMENS",
                      desc: "Busca la referencia en el catálogo online de Siemens.",
                    },
                    {
                      op: "Buscar equivalencia Competencia",
                      desc: "Busca el equivalente de competencia para la referencia.",
                    },
                    {
                      op: "Calcular PVP a partir de GA",
                      desc: "Calcula el PVP desde el precio GA aplicando el factor.",
                    },
                    {
                      op: "Asistente Referencias",
                      desc: "Asistente guiado para construir referencias Siemens.",
                    },
                    {
                      op: "Guardar Imagen BD",
                      desc: "Asocia una imagen al producto en BD.",
                    },
                    {
                      op: "Buscar Subfamilia del Descuento",
                      desc: "Identifica la subfamilia de descuento para la referencia.",
                    },
                  ],
                },
                {
                  id: "m-clientes",
                  titulo: "Menú Clientes",
                  color: "#171717",
                  items: [
                    {
                      op: "Gestionar Clientes",
                      desc: "Abre un diálogo con la lista de clientes editable. Permite modificar cualquier campo (excepto el ID), copiar un cliente para crear uno nuevo y guardar todos los cambios en BD en una sola transacción con confirmación. Marca en amarillo los clientes modificados y en verde los nuevos.",
                    },
                  ],
                },
                {
                  id: "m-descuentos",
                  titulo: "Menú Descuentos",
                  color: "#171717",
                  items: [
                    {
                      op: "Aplicar descuentos",
                      desc: "Lista las combinaciones únicas de Grupo Descuento + descuento actual del presupuesto (líneas PD/PE/E). Muestra grupo, familia, subfamilia, número de líneas, descuento actual e importe neto acumulado. Permite elegir una combinación y aplicar un nuevo descuento a todas sus líneas.",
                    },
                    {
                      op: "Calcular Descuento",
                      desc: "Recalcula los descuentos según la tarifa activa.",
                    },
                    {
                      op: "Fijar el precio total",
                      desc: "Pide el importe neto total objetivo y reparte el descuento entre todas las líneas del presupuesto proporcionalmente a su neto actual, ajustando el descuento de cada una para conseguir ese total.",
                    },
                    {
                      op: "Fijar precio de celdas",
                      desc: "Pide el importe neto objetivo y lo reparte solo entre las filas seleccionadas (por checkbox o por rango de celdas) proporcionalmente a su neto actual.",
                    },
                  ],
                },
                {
                ...
                <section key={menu.id} id={"ayuda-" + menu.id}>
                  <h2
                    style={{
                      ...
                      color: menu.color,
                      marginBottom: 12,
                      borderBottom: `2px solid ${menu.color}`,
                      paddingBottom: 8,
                    }}
                  >
                    {menu.titulo}
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Icon as={BarChart3} size={18} color="#171717" /> Resumen del
            Presupuesto
          </span>
          <span style={{ color: "#737373", fontSize: 12 }}>
            {presupuesto.np} — {presupuesto.titulo}
          </span>
        </div>
        <div
          style={{
        ...
        >
          <div style={{ flex: 1, minWidth: 380 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>
              Desglose por Familia / SubFamilia
            </h3>
            <div
              style={{
                border: "1px solid #e2e8f0",
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {resumenData.map((row, i) => (
                    ...
                    >
                      ...
                      >
                        {totalResumen
                          ? ((row.importe / totalResumen) * 100).toFixed(1)
                          : 0}
                        %
                      </td>
                    </tr>
                ...
                <tfoot>
                  ...
                  >
                    ...
                    >
                      {resumenData.reduce((s, r) => s + r.articulos, 0)}
                    </td>
                    <td
                      style={{
                        padding: "8px 12px",
          ...
          >
            ...
            <DonutChart
              data={resumenData}
              total={totalResumen}
              onHover={setDonutTooltip}
              onLeave={() => setDonutTooltip(null)}
            />
            <div
              style={{
                display: "flex",
        ...
        {donutTooltip && (
          ...
          >
            <div style={{ fontWeight: 600 }}>
              {donutTooltip.item.subfamilia}
            </div>
            <div style={{ color: "#93c5fd" }}>{donutTooltip.item.familia}</div>
            <div style={{ marginTop: 4 }}>
              {fmt(donutTooltip.item.importe)} €{" "}
              <span style={{ color: "#94a3b8" }}>
                ({(donutTooltip.pct * 100).toFixed(1)}%)
              </span>
            </div>
            <div style={{ color: "#94a3b8" }}>
              {donutTooltip.item.articulos} artículos
            </div>
          </div>
        )}
      </div>
  ...
  if (showWelcome) {
    const entrar = (userInfo) => {
      ...
      if (userInfo && userInfo.usuario) {
        ...
        if (userInfo.codigopresupuestos)
          setCodigoUsuario(userInfo.codigopresupuestos);
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Icon as={FileSpreadsheet} size={18} color="#171717" /> Presupuestos
        </span>
        <span style={{ color: "#737373", fontSize: 12 }}>
          v1.62.2 (1 Junio 2026)
        </span>
        {estructuraActiva && (
          ...
          >
            <Icon as={Palette} size={12} color="#14532d" /> Estructura activa
          </span>
        )}
          ...
          {userMenuOpen && (
            <>
              ...
              >
                ...
                <div
                  style={{ height: 1, background: "#e5e5e5", margin: "2px 0" }}
                />
                <button
                  onClick={() => {
                    setUserMenuOpen(false);
                    setUsuarioActual("");
                    setCodigoUsuario("CDM");
                  ...
                  style={{
                    ...
                    border: "none",
                    background: "transparent",
                    cursor: usuarioActual ? "pointer" : "default",
                    fontSize: 12,
                    color: usuarioActual ? "#171717" : "#94a3b8",
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                  }}
                ...
                >
                  <Icon
                    as={LogOut}
                    size={14}
                    color={usuarioActual ? "#dc2626" : "#94a3b8"}
                  />{" "}
                  Cerrar sesión
                </button>
              </div>
      ...
      >
        {MENU_STRUCTURE.map((group) => (
          <MenuGroup key={group.id} group={group} onAction={handleAction} />
        ))}
        <div
          style={{
            marginLeft: "auto",
        ...
        >
          <span>
            Neto:{" "}
            <strong style={{ color: "#171717" }}>{fmt(totalNeto)} €</strong>
          </span>
          <span>
            Coste:{" "}
            <strong style={{ color: "#171717" }}>{fmt(totalCoste)} €</strong>
          </span>
          <span>
            Margen:{" "}
            <strong style={{ color: margenTotal > 30 ? "#16a34a" : "#dc2626" }}>
              {fmt(margenTotal)}%
            </strong>
          </span>
        </div>
      ...
      >
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setPresupuesto((p) => {
                  ...
                  if (f.key === "np" || f.key === "anopresupuesto") {
                    updated.numerocompleto = buildNumeroCompleto(
                      codigoUsuario,
                      updated.np,
                      updated.anopresupuesto
                    );
                  }
                  return updated;
                });
              ...
              style={{
                ...
                background: f.readonly ? "#f1f5f9" : "#f8fafc",
                cursor: f.readonly ? "default" : "text",
                fontWeight: f.readonly ? 600 : 400,
              }}
            />
            {f.key === "numerocompleto" && (
              <button
                onClick={() => {
                  const nuevo = buildNumeroCompleto(
                    codigoUsuario,
                    presupuesto.np,
                    presupuesto.anopresupuesto
                  );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button
            onClick={() => setShowSelecCliente(true)}
          ...
          >
            ...
            {presupuesto.idcliente
              ? presupuesto.cliente || "Cliente sin nombre"
              : "Seleccionar cliente"}
          </button>
          {presupuesto.idcliente && (
            <button
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button
            onClick={() => setShowSelecContacto(true)}
          ...
          >
            ...
            {presupuesto.alaatencion
              ? `${presupuesto.alaatencion_nombre || "Contacto"}${
                  presupuesto.alaatencion_cliente
                    ? " - " + presupuesto.alaatencion_cliente
                    : ""
                }`
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <select
            ...
            onChange={(e) =>
              setPresupuesto((p) => ({
                ...p,
                idpaisclientefinal: e.target.value
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <input
            ...
            onChange={(e) =>
              setPresupuesto((p) => ({ ...p, clientefinal: e.target.value }))
            }
        ...
        >
            ...
            <tr>
              ...
              {COLUMNS.map((col) => {
                ...
                return (
                  ...
                  >
                    {Array.isArray(col.label)
                      ? col.label.map((l, i) => <div key={i}>{l}</div>)
                      : col.label}
          ...
          <tbody>
            {rows.map((row, rowIdx) => {
              ...
              const esSubtotal =
                SUBTOTAL_NIVELES[row.naturaleza] !== undefined || esTotal;
              const subtotalCalc = esTotal
                ? calcTotalPresupuesto(rows)
                : SUBTOTAL_NIVELES[row.naturaleza] !== undefined
                ? calcSubtotal(rows, rowIdx, SUBTOTAL_NIVELES[row.naturaleza])
                : null;
              ...
              const colsConEstilo = estructuraActiva
                ? estilo.isTitle
                  ? COLS_ESTILO_TITULO
                  : estilo.isSubtotal
                  ? COLS_ESTILO_SUBTOTAL
                  : estilo.isComment
                  ? COLS_ESTILO_COMENT
                  : null
                : null;
              const aplicarEstiloFila = !colsConEstilo; // null → estilo en toda la fila
              const rowBgDefault = selectedRows.has(row.id)
                ? "#dbeafe"
                : rowIdx % 2 === 0
                ? "#fff"
                : "#f8fafc";
              const rowBg = aplicarEstiloFila
                ? selectedRows.has(row.id)
                  ? "#dbeafe"
                  : estilo.bg || rowBgDefault
                : rowBgDefault;
              return (
                <tr
                  key={row.id}
                  style={{
                    background: rowBg,
                    height: altosManual[row.id] || undefined,
                  }}
                >
                  <td
                    style={{
                  ...
                  {COLUMNS.map((col) => {
                    ...
                    const isRight =
                      col.type === "number" || col.type === "calc";
                    const isHidden =
                      colsVisibles && !colsVisibles.includes(col.key);
                    let displayVal = row[col.key] ?? "";
                    if (col.key === "posicion" && estructuraActiva)
                      displayVal = apartados[row.id] ?? row.posicion ?? "";
                    ...
                    if (col.key === "precionetounitario")
                      displayVal = estructuraActiva
                        ? fmtEur(calcNetoUnit(row))
                        : fmt(calcNetoUnit(row));
                    if (col.key === "precionetoposicion") {
                      const val = esSubtotal
                        ? subtotalCalc.neto
                        : calcNetoPos(row);
                      displayVal = estructuraActiva ? fmtEur(val) : fmt(val);
                    }
                    if (col.key === "dtoaplicado" && esSubtotal)
                      displayVal = fmt(subtotalCalc.dto) + "%";
                    else if (
                      col.key === "dtoaplicado" &&
                      estructuraActiva &&
                      typeof displayVal === "number"
                    )
                      displayVal = fmt(displayVal) + "%";
                    if (col.key === "costeposicion")
                      displayVal = estructuraActiva
                        ? fmtEur(calcCostePos(row))
                        : fmt(calcCostePos(row));
                    if (col.key === "margen")
                      displayVal = fmt(calcMargen(row)) + "%";
                    if (
                      col.key === "cantidad" &&
                      typeof displayVal === "number"
                    )
                      displayVal = displayVal.toLocaleString("es-ES");
                    ...
                    if (
                      ...
                      displayVal = fmtEur(displayVal);
                    if (
                      estructuraActiva &&
                      col.key === "preciocosteunitario" &&
                      typeof displayVal === "number"
                    )
                      displayVal = fmtEur(displayVal);
                    if (
                      estructuraActiva &&
                      col.key === "precionetounitario2" &&
                      typeof displayVal === "number"
                    )
                      displayVal = fmtEur(displayVal);
                    else if (
                    ...
                    const celdaConEstilo = colsConEstilo
                      ? colsConEstilo.includes(col.key)
                      : aplicarEstiloFila;
                    ...
                    return (
                      <td
                        ...
                        onMouseEnter={() => {
                          if (isSelecting) {
                            setSelectionRange((r) =>
                              r
                                ? { ...r, endRowIdx: rowIdx, endColIdx: colIdx }
                                : null
                            );
                          }
                        }}
                        ...
                        style={{
                          ...
                          background:
                            inRange && !isSelected ? "#dbeafe" : cellBg,
                          cursor: isCalc || isHidden ? "default" : "cell",
                          verticalAlign: "middle",
                          overflow:
                            isEditing &&
                            (col.type === "select" ||
                      ...
                      >
                        {isHidden ? null : isEditing &&
                          ...
                          (() => {
                            const opciones =
                              col.opciones === "naturaleza"
                                ...
                                : [
                                    {
                                      value: "",
                                      label: "( Sin representación )",
                                      desc: "",
                                    },
                                    ...REPRESENTACION_OPCIONES.map((n) => ({
                                      value: n.representacion,
                                      label: n.representacion,
                                      desc: n.descripcion,
                                    })),
                                  ];
                            const filtroTxt = String(editValue || "")
                              .toUpperCase()
                              .trim();
                            ...
                            const tryConfirm = () => {
                              ...
                              if (filtroTxt === "") {
                                setRows((rs) =>
                                  rs.map((rr) =>
                                    rr.id === editingCell.rowId
                                      ? { ...rr, [editingCell.colKey]: "" }
                                      : rr
                                  )
                                );
                                setEditingCell(null);
                                ...
                                setRows((rs) =>
                                  rs.map((rr) =>
                                    rr.id === editingCell.rowId
                                      ? {
                                          ...rr,
                                          [editingCell.colKey]: exacta.value,
                                        }
                                      : rr
                                  )
                                );
                            ...
                            (() => {
                              ...
                              if (esTextoLargo) {
                                ...
                                const charsPorLinea = Math.max(
                                  20,
                                  Math.floor((anchoEditor - 20) / 6.5)
                                );
                                const lineasTxt = txt.split("\n");
                                let lineasEstimadas = 0;
                                lineasTxt.forEach((l) => {
                                  lineasEstimadas += Math.max(
                                    1,
                                    Math.ceil(l.length / charsPorLinea)
                                  );
                                });
                                const rowsCalc = Math.min(
                                  Math.max(3, lineasEstimadas + 1),
                                  20
                                );
                                return (
                                  <textarea
                          ...
                          (() => {
                            const esColTextoEstructura =
                              (estructuraActiva || wrapPersistente) &&
                              COLS_WRAP_ESTRUCTURA.includes(col.key);
                            const haceWrap =
                              (col.wrap || esColTextoEstructura) && !descCorta;
                            ...
                            if (
                              estructuraActiva &&
                              COLS_IMPORTE.includes(col.key)
                            ) {
                              if (col.key === "precionetounitario")
                                valNum = calcNetoUnit(row);
                              else if (col.key === "precionetoposicion")
                                valNum = esSubtotal
                                  ? subtotalCalc.neto
                                  : calcNetoPos(row);
                              else if (col.key === "costeposicion")
                                valNum = calcCostePos(row);
                              else valNum = Number(row[col.key]) || 0;
                            }
                            const importeNegativo =
                              valNum != null && valNum < 0;
                            const tooltipFull =
                              col.key === "subfamilia" &&
                              row.descripcionsubfamilia
                                ? (row.subfamilia || "") +
                                  ": " +
                                  row.descripcionsubfamilia
                                : col.key === "grupodescuento" &&
                                  row.descripciongrupodescuento
                                ? (row.grupodescuento || "") +
                                  ": " +
                                  row.descripciongrupodescuento
                                : [
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  padding: "4px 8px",
                                  fontSize:
                                    celdaConEstilo && estilo.fontSize
                                      ? estilo.fontSize
                                      : 12,
                                  ...
                                  textAlign:
                                    estructuraActiva &&
                                    col.key === "nombre" &&
                                    (estilo.isTitle || estilo.isComment)
                                      ? "center"
                                      : estructuraActiva &&
                                        col.key === "nombre" &&
                                        estilo.isSubtotal
                                  ...
                                  color: importeNegativo
                                    ...
                                    : "#1e293b",
                                  fontFamily:
                                    celdaConEstilo && estilo.fontFamily
                                      ? estilo.fontFamily
                                      : "inherit",
                                  fontWeight:
                                    celdaConEstilo &&
                                    (estilo.isTitle || estilo.isSubtotal)
                                      ? estilo.fontWeight
                                      : isCalc
                                      ? 500
                                      : 400,
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          >
            <BtnContent icon={Plus} iconColor="#16a34a">
              Nueva fila
            </BtnContent>
          </button>
          <input
          ...
          <button
            onClick={() => {
              ...
              if (idsFila.size === 0) {
                setStatus(
                  "Selecciona al menos una celda o rango para marcar sus filas",
                  "error"
                );
                return;
              }
              ...
              setStatus(
                `Marcadas ${idsFila.size} fila${idsFila.size !== 1 ? "s" : ""}`,
                "success"
              );
            }}
            ...
            style={{
              padding: "7px 10px",
              fontSize: 12,
              cursor: !selectionRange && !selectedCell ? "default" : "pointer",
              borderRadius: 4,
              border: "1px solid #cbd5e1",
              background: !selectionRange && !selectedCell ? "#fafafa" : "#fff",
              color: !selectionRange && !selectedCell ? "#cbd5e1" : "#1e293b",
              whiteSpace: "nowrap",
            }}
          >
            <BtnContent
              icon={Check}
              iconColor={
                !selectionRange && !selectedCell ? "#cbd5e1" : "#16a34a"
              }
            >
              Marcar Celdas
            </BtnContent>
          ...
          <button
            ...
            style={{
              padding: "7px 10px",
              fontSize: 12,
              cursor: selectedRows.size === 0 ? "default" : "pointer",
              borderRadius: 4,
              border: "1px solid #fca5a5",
              background: selectedRows.size === 0 ? "#fafafa" : "#fff",
              color: selectedRows.size === 0 ? "#cbd5e1" : "#dc2626",
              whiteSpace: "nowrap",
            }}
          >
            <BtnContent
              icon={Trash2}
              iconColor={selectedRows.size === 0 ? "#cbd5e1" : "#dc2626"}
            >
          ...
          <button
            ...
            title={
              selectedRows.size > 0
                ? "Borra el contenido de todas las celdas de las filas seleccionadas"
                : "Borra el contenido de las celdas seleccionadas (no la fila)"
            }
            style={{
              padding: "7px 10px",
              fontSize: 12,
              cursor:
                !selectionRange && selectedRows.size === 0
                  ? "default"
                  : "pointer",
              borderRadius: 4,
              border: "1px solid #cbd5e1",
              background:
                !selectionRange && selectedRows.size === 0 ? "#fafafa" : "#fff",
              color:
                !selectionRange && selectedRows.size === 0
                  ? "#cbd5e1"
                  : "#1e293b",
          ...
          >
            ...
            >
              Borrar contenido celdas
              {selectedRows.size > 0 ? ` (${selectedRows.size} filas)` : ""}
            </BtnContent>
          </button>
          <div
            style={{
          ...
          <button
            onClick={() => {
              if (selectedRows.size === 0) {
                setStatus(
                  "Marca con checkbox las filas que quieres copiar",
                  "error"
                );
                return;
              }
              ...
              setStatus(
                `${copia.length} fila${copia.length !== 1 ? "s" : ""} copiada${
                  copia.length !== 1 ? "s" : ""
                } al portapapeles interno`,
                "success"
              );
            }}
            disabled={selectedRows.size === 0}
            ...
            style={{
              ...
              border: "1px solid #cbd5e1",
              background: selectedRows.size === 0 ? "#fafafa" : "#fff",
              color: selectedRows.size === 0 ? "#94a3b8" : "#1e293b",
              whiteSpace: "nowrap",
            }}
          >
            <BtnContent
              icon={Copy}
              iconColor={selectedRows.size === 0 ? "#cbd5e1" : "#0369a1"}
            >
              Copiar filas
              {selectedRows.size > 0 ? ` (${selectedRows.size})` : ""}
            </BtnContent>
          ...
          <button
            onClick={() => {
              if (filasCopiadas.length === 0) {
                setStatus(
                  "No hay filas copiadas. Marca filas con checkbox y pulsa 'Copiar filas'",
                  "error"
                );
                return;
              }
              if (!selectedCell) {
                setStatus(
                  "Selecciona una celda para indicar dónde pegar",
                  "error"
                );
                return;
              }
              const idxDestino = rows.findIndex(
                (r) => r.id === selectedCell.rowId
              );
              if (idxDestino === -1) {
                setStatus(
                  "La celda seleccionada no tiene fila válida",
                  "error"
                );
                return;
              }
              ...
              setRows((rs) => {
                ...
                const aPegar = filasCopiadas.map((f) => ({
                  id: nextLocalId++,
                  ...f,
                }));
                ...
                for (let i = idxDestino; i <= finDestino; i++) {
                  if (i >= res.length || !esFilaVacia(res[i])) {
                    todasVacias = false;
                    break;
                  }
                }
              ...
              setStatus(
                `${filasCopiadas.length} fila${
                  filasCopiadas.length !== 1 ? "s" : ""
                } pegada${
                  filasCopiadas.length !== 1 ? "s" : ""
                } a partir de la fila ${idxDestino + 1}`,
                "success"
              );
            }}
            ...
            style={{
              ...
              fontSize: 12,
              cursor:
                filasCopiadas.length === 0 || !selectedCell
                  ? "default"
                  : "pointer",
              borderRadius: 4,
              border: "1px solid #cbd5e1",
          ...
          >
            ...
            >
              Pegar filas
              {filasCopiadas.length > 0 ? ` (${filasCopiadas.length})` : ""}
            </BtnContent>
          </button>
          <div
            style={{
          ...
          <button
            onClick={() => {
              ...
              if (ok) {
                setStatus(
                  `Presupuesto guardado en el navegador (${rows.length} filas)`,
                  "success"
                );
              } else {
                setStatus(
                  "Error al guardar el presupuesto en el navegador",
                  "error"
                );
              }
            }}
          ...
          >
            <BtnContent icon={Save} iconColor="#16a34a">
              Guardar local
            </BtnContent>
          </button>
        </div>
      ...
      >
        <span>
          Filas: <strong style={{ color: "#171717" }}>{rows.length}</strong>
        </span>
        <span>
          Seleccionadas:{" "}
          <strong style={{ color: "#171717" }}>{selectedRows.size}</strong>
        </span>
        {selectedCell && (
          <span>
            Celda:{" "}
            <strong style={{ color: "#171717" }}>
              {selectedCell.colKey} / fila{" "}
              {rows.findIndex((r) => r.id === selectedCell.rowId) + 1}
            </strong>
          </span>
        )}
        <span style={{ marginLeft: "auto", fontSize: 11, color: "#737373" }}>
      ...
      {showFijarPrecio &&
        (() => {
          ...
          if (showFijarPrecio === "total") {
            lineasAfectadas = rows.filter((r) =>
              ["PD", "PE", "E"].includes(r.naturaleza)
            );
            titulo = "Fijar el precio total del presupuesto";
            descripcion =
              "Indica el importe neto total objetivo. Se ajustará el descuento de todas las líneas de producto/elemento proporcionalmente a su neto actual.";
          } else {
            ...
            lineasAfectadas = rows.filter(
              (r) =>
                idsAfectadas.has(r.id) &&
                ["PD", "PE", "E"].includes(r.naturaleza)
            );
            titulo = "Fijar el precio de las celdas seleccionadas";
            descripcion =
              "Indica el importe neto objetivo para las filas seleccionadas. Se ajustará el descuento de todas ellas proporcionalmente a su neto actual. (Solo afecta a líneas de producto/elemento)";
          }
          return (
            <FijarPrecioDialog
              titulo={titulo}
              ...
              onAplicar={(nuevosDescuentos) => {
                setRows((rs) =>
                  rs.map((r) =>
                    nuevosDescuentos.hasOwnProperty(r.id)
                      ? { ...r, dtoaplicado: nuevosDescuentos[r.id] }
                      : r
                  )
                );
              }}
      ...
      {confirmBorrarPresup && (
        ...
        >
          ...
          >
            ...
            >
              Se va a borrar el presupuesto actual (cabecera y todas las filas)
              y se preparará uno nuevo en blanco con el siguiente número
              disponible.
              <br />
              <span style={{ color: "#dc2626", fontSize: 12 }}>
                Esta acción no se puede deshacer.
            ...
            >
              <button
                onClick={() => setConfirmBorrarPresup(false)}
                style={{
                  padding: "7px 16px",
                  borderRadius: 6,
                  border: "1px solid #cbd5e1",
              ...
              <button
                onClick={async () => {
                  ...
                  setStatus(
                    "Presupuesto borrado, obteniendo siguiente número...",
                    "working"
                  );
                  try {
                    ...
                    setStatus(
                      `Presupuesto en blanco. ${buildNumeroCompleto(
                        codigoUsuario,
                        sig,
                        anoActual
                      )}`,
                    ...
                    setStatus(
                      "Presupuesto borrado, pero no se ha podido obtener el siguiente número: " +
                        e.message,
                      "error"
                    );
                  }
        ...
        >
          ...
          >
            ...
            {comprobarDialog.tipo === "no_existe" && (
              ...
              >
                <strong>
                  El presupuesto Nº {presupuesto.np} Rev.{presupuesto.revision}
                </strong>{" "}
                no existe todavía en la base de datos. Si pulsas{" "}
                <em>Guardar Presupuesto</em> se creará por primera vez.
              </p>
            )}
            ...
            {comprobarDialog.tipo === "diferentes" && (
              <div style={{ overflow: "auto", flex: 1 }}>
                ...
                >
                  <strong>
                    {comprobarDialog.diffs.length} diferencia
                    {comprobarDialog.diffs.length > 1 ? "s" : ""}
                  </strong>{" "}
                  entre el presupuesto actual y el guardado en BD:
                </div>
                <ul
                  style={{
                    margin: 0,
            ...
            >
              <button
                onClick={() => setComprobarDialog(null)}
                style={{
                  padding: "7px 20px",
                  borderRadius: 6,
                  border: "none",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Sobrescribir presupuesto existente
              </h3>
            </div>
            <p
              style={{
            ...
            >
              Ya existe un presupuesto{" "}
              <strong>{presupuesto.numerocompleto || presupuesto.np}</strong>{" "}
              Rev.<strong>{presupuesto.revision}</strong> en la base de datos
              (ID {confirmSobreescribir.idBd}, {confirmSobreescribir.lineasBd}{" "}
              líneas).
            </p>
            <p
              style={{
            ...
            >
              Se han detectado{" "}
              <strong>
                {confirmSobreescribir.diffs.length} diferencia
                {confirmSobreescribir.diffs.length > 1 ? "s" : ""}
              </strong>{" "}
            ...
            >
              <strong>Atención:</strong> al sobrescribir, los datos guardados en
              BD se sustituirán por los del presupuesto actual y no se podrán
              recuperar.
            </div>
            <div
              style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}
            >
              <button
                onClick={() => setConfirmSobreescribir(false)}
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const r = await guardarPresupuestoEnBD(
                      confirmSobreescribir.idBd
                    );
                    setPresupuesto((p) => ({
                      ...p,
                      id: String(confirmSobreescribir.idBd),
                    }));
                    setStatus(
                      `Presupuesto actualizado en BD (${r.lineas_actualizadas} líneas)`,
                      "success"
                    );
                ...
                style={{
                  ...
                  background: "#dc2626",
                  color: "#fff",
                  cursor: guardandoPresup ? "default" : "pointer",
                  fontSize: 13,
                  fontWeight: 600,
                }}
              >
                <BtnContent
                  icon={guardandoPresup ? RefreshCw : Save}
                  iconColor="#fff"
                >
                  {guardandoPresup ? "Guardando..." : "Sí, sobrescribir"}
                </BtnContent>
              </button>
            </div>
          </div>
          ...
          onInsertar={(prod) => {
            ...
            setRows((r) => {
              ...
              const idx = selectedCell
                ? r.findIndex((row) => row.id === selectedCell.rowId)
                : -1;
              if (idx === -1) {
      ...
      {showLeerElem && (
        <LeerElementoDialog
          ...
          onInsertar={(data) => {
            ...
            console.log(
              "[LeerElemento] productos recibidos del backend:",
              data.productos
            );
            const filasProducto = (data.productos || []).map((p) => {
              ...
              const esComentario =
                p.es_comentario === true ||
                p.escomentario === true ||
                p.eselemento === false ||
                Number(p.idproducto) === 118852;
              if (esComentario) {
                return {
                  id: nextLocalId++,
                  representacion: "",
      ...
      {juntarDupConfirm && (
        <div
          ...
          onClick={() => setJuntarDupConfirm(null)}
        >
          <div
            style={{
              background: "#fff",
          ...
          >
            ...
            >
              <Icon as={Layers} size={18} color="#1e3a5f" /> Juntar productos
              duplicados
            </h3>
            <p
              style={{
            ...
            >
              Se han encontrado{" "}
              <strong style={{ color: "#171717" }}>
                {juntarDupConfirm.totalGrupos} referencia
                {juntarDupConfirm.totalGrupos !== 1 ? "s" : ""} duplicada
                {juntarDupConfirm.totalGrupos !== 1 ? "s" : ""}
              </strong>{" "}
              en el rango seleccionado, afectando a{" "}
              <strong style={{ color: "#171717" }}>
                {juntarDupConfirm.totalFilasAfectadas} fila
                {juntarDupConfirm.totalFilasAfectadas !== 1 ? "s" : ""}
              </strong>
              .
            </p>
            <div
            ...
            >
              {juntarDupConfirm.duplicados.map(([ref, idxs]) => {
                const totalCant = idxs.reduce(
                  (s, i) => s + (Number(rows[i]?.cantidad) || 0),
                  0
                );
                return (
                  <div
                  ...
                  >
                    ...
                    <span style={{ color: "#0369a1" }}>
                      {idxs.length} × → cant. total {totalCant}
                    </span>
                  </div>
                );
              })}
            ...
            <p style={{ fontSize: 12, color: "#525252", marginBottom: 14 }}>
              La cantidad total se sumará en la <strong>primera fila</strong> de
              cada grupo. ¿Qué hacemos con las filas duplicadas restantes?
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
                ...
                onClick={() => {
                  ...
                  setRows((prev) => {
                    ...
                    juntarDupConfirm.duplicados.forEach(([_, idxs]) => {
                      const totalCant = idxs.reduce(
                        (s, i) => s + (Number(next[i]?.cantidad) || 0),
                        0
                      );
                  ...
                  setStatus(
                    `Juntados ${juntarDupConfirm.totalGrupos} grupo${
                      juntarDupConfirm.totalGrupos !== 1 ? "s" : ""
                    } duplicado${
                      juntarDupConfirm.totalGrupos !== 1 ? "s" : ""
                    }, ${
                      juntarDupConfirm.totalFilasAfectadas -
                      juntarDupConfirm.totalGrupos
                    } fila${
                      juntarDupConfirm.totalFilasAfectadas -
                        juntarDupConfirm.totalGrupos !==
                      1
              ...
              >
                <BtnContent icon={Check} iconColor="#475569">
                  Dejar a 0
                </BtnContent>
              </button>
              <button
          ...
          aplicarAFila={(rowIdx, datos) => {
            setRows((rs) => {
              ...
              if (datos.descripcion != null)
                upd.descripcion = String(datos.descripcion);
              if (datos.pvp != null) {
              ...
              if (datos.preciocoste != null) {
                const n = parseFloat(
                  String(datos.preciocoste).replace(",", ".")
                );
      ...
      {showLoginDialog && (
        <LoginDialog
          ...
          onLoginOk={(userData) => {
            ...
            if (userData.codigopresupuestos)
              setCodigoUsuario(userData.codigopresupuestos);
        ...
        <SiemensRefsDialog
          ...
          onComplete={(aceptadas) => {
            ...
            setRows((prev) => {
              ...
              newRows.forEach((r) => {
                finales.push(r);
                const nuevas = nuevasFilas.filter(
                  (n) => n.pivoteRowId === r.id
                );
        ...
        <SelectorContactoDialog
          ...
          onSelect={(contacto) => {
            setPresupuesto((p) => ({
              ...
              alaatencion_cliente:
                contacto.cliente_nombre || contacto.cliente_razonsocial || "",
            }));
      ...
      {showLeer && (
        <LeerPresupuestosDialog
          ...
          onCargar={(data) => {
            setStatus(
              `Presupuesto ${
                data.cabecera.numerocompleto || data.cabecera.numero
              } cargado: ${(data.detalle || []).length} líneas`,
              "success"
            );
            // Cargar cabecera
            ...
            setPresupuesto({
              ...
              alaatencion_cliente:
                cab.alaatencion_cliente_nombre || cab.alaatencion_cliente || "",
            });
          ...
          onApply={(grupodescuento, dtoActual, dto) => {
            setRows((r) =>
              r.map((row) => {
                const gd = (row.grupodescuento || "").trim() || "(Sin grupo)";
                const dtoRow =
                  Math.round((Number(row.dtoaplicado) || 0) * 100) / 100;
                const matchGrupo = gd === grupodescuento;
                const matchDto = dtoRow === dtoActual;
                if (
                  matchGrupo &&
                  matchDto &&
                  ["PD", "PE", "E"].includes(row.naturaleza)
                ) {
        ...
        >
          ...
          >
            ...
            >
              ...
              <h2 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>
                Confirmar borrado de contenido
              </h2>
            </div>
            <p style={{ fontSize: 13, color: "#475569", margin: "0 0 20px" }}>
              ¿Desea borrar el contenido de todas las celdas de{" "}
              <strong>{selectedRows.size}</strong> fila
              {selectedRows.size > 1 ? "s" : ""} seleccionada
              {selectedRows.size > 1 ? "s" : ""}?
              <br />
              <span style={{ color: "#94a3b8", fontSize: 12 }}>
                Las filas se mantendrán pero quedarán vacías.
              </span>
            </p>
            <div
              style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}
            >
              ...
              >
                <BtnContent icon={Eraser}>Borrar contenido</BtnContent>
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmDelete && (
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 13, color: "#475569", margin: "0 0 20px" }}>
              ¿Desea borrar <strong>{selectedRows.size}</strong> fila
              {selectedRows.size > 1 ? "s" : ""}?
              <br />
              <span style={{ color: "#dc2626", fontSize: 12 }}>
                Esta acción no se puede deshacer.
            ...
            >
              ...
              <button
                onClick={() => {
                  setRows((r) => r.filter((row) => !selectedRows.has(row.id)));
                  setSelectedRows(new Set());
              ...
              >
                <BtnContent icon={Trash2}>Borrar</BtnContent>
              </button>
            </div>
          </div>
        </div>
