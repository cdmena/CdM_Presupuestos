...
import {
  FileText, FolderOpen, Download, Printer, BarChart3, Palette,
  Grid3x3, Search, Eraser, MoreHorizontal, Calculator, Link2,
  Eye, Trash2, X, Scale, Square, MessageSquare, Plus, FileInput,
  FileX, Edit3, Inbox, Euro, TrendingUp, Scissors, CornerDownLeft,
  Folder, DollarSign, Database, Repeat, Bot, Image as ImageIcon,
  HelpCircle, Settings, Percent, Users, Target, Hash, ChevronRight,
  Save, RefreshCw, Home, FileSpreadsheet, MousePointer, Layers,
  Package, Wrench, ArrowLeft, Check, Copy, ListPlus, FileUp,
  ClipboardCheck, User, Lock, LogIn, LogOut
} from "lucide-react";
...
function parsePastedTSV(text) {
  ...
  return lines.map(l => l.includes("\t") ? l.split("\t") : l.includes(";") ? l.split(";") : l.split(","));
}
...
function OpcionesScreen({ estilos, setEstilos, onVolver, setStatus, statusMessage }) {
...
function exportToExcel(presupuesto, rows, apartados, estructuraActiva, estilos) {
  ...
  const nombreFichero = `Oferta SIEMENS ${presupuesto.np} - ${presupuesto.titulo} - ${presupuesto.cliente}`
    .replace(/[\\/:*?"<>|]/g, "-");
  ...
  if (vista === "ayuda") return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", fontSize: 13, color: "#1e293b", height: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ background: "#f5f5f5", color: "#171717", padding: "8px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0, borderBottom: "1px solid #e5e5e5" }}>
        ...
        <span style={{ color: "#737373", fontSize: 12 }}>v1.62.2 (1 Junio 2026)</span>
      </div>
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        {/* ÁRBOL IZQUIERDA */}
